...
**Write ordering.** The writer validates the plan before destructive writes, then writes prefix, volatile tail, dispatch plan and JSON context completion marker LAST. Malformed angle/model/capability inputs fail closed. A mid-set write failure must never leave a complete-looking context pointing at missing or stale siblings. The writer removes the previous JSON marker when prefix bytes change, retaining it for same-bytes reruns; `readGateContext` and reviewer `--context-path` checks use that marker.
