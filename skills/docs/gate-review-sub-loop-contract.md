...
- `file`: an orchestrator-supplied `--prefix-file` is recorded verbatim. The artifact discloses the mode; the supplied text gains no mode line, and no evidence file is written; `requiredReads` carries only a `prior-dispositions` entry when a `--prev-head` rebuild seeds one.
