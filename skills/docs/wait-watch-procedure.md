...
<!-- pi-only -->
Under Pi, a bounded child exits on external wait when `cycleDisposition` is
`pending` and `terminal` is `false`; the main session re-dispatches the same target
when feasible. Carry the checkpoint and watch outcome, and report the pending
boundary accurately rather than declaring completion.
<!-- /pi-only -->
