...
11b. <!-- rule: LOCAL-PRE-PR-REVIEW-BEFORE-PUSH --> **Pre-PR review pass (before the first push).** For tracker-backed sessions, before pushing the branch and before PR creation, run the pre-PR review phase per [Pre-PR review contract](../docs/pre-pr-review-contract.md): hand a mandatory developer-authored review brief to ONE fresh-context general-purpose reviewer (model resolved via `resolveRoleModel(config, { role: "pre-PR-reviewer", harness })`, passed only when non-null), paired with the adversarial-enumeration checklist; apply findings in-tree; bounded to one reviewer and at most two rounds; then push once. The pass is ephemeral (no PR/comment/thread/Copilot) and is NOT lifecycle-gate evidence. The full fan-out `draft_gate`/`pre_approval_gate` still runs afterward as the authority.
