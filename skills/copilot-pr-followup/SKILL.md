...
4. **Fan-out (Phase 2):** plan batches with `planFanoutBatches(angles, cap)` using the `gates.maxFanoutReviewers` cap (default 8), over the current head's resolved angle set minus the plan's `carried` angles (the Phase 1.2 subtraction — never `mustRerun`); spawn one scoped `review` agent per such angle (plain Agent tool), parallel up to the cap, sequential for overflow (record any degradation in the gate evidence). Each reviewer follows [review agent scoped angle-review mode](../../agents/review.md): fresh context, single angle, read-only, writing its per-angle findings artifact to `tmp/gate-reviews/<repo-slug>/pr-<N>/<gate>-<headSha>/<angle>.json`.
   ...
   reviewer does not re-raise what a thread already covers; the block's content, dedupe
   contract, and prefix-hash non-interference are owned by `GATE-EXEC-FINDING-THREADS` in
