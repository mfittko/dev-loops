...
- **Persona mapping:** each angle resolves to a reviewer persona via `resolveReviewerRole(config, angle)` from `@dev-loops/core/config`. The fan-out emitter carries each resolved prompt in the unit's work order (`angleInstructions`) so the reviewer knows exactly what to look for.
<!-- pi-only -->
