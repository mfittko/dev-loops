...
<!-- pi-only -->
- **Model tier:** resolve each reviewer's model via `resolveRoleModel(config, { role: angle, harness: "pi" })` from `@dev-loops/core/config` (critical angles resolve to the high tier via their `review` persona) and pass it at dispatch **only when non-null** — a no-op on Pi until an operator sets `models.tiers.<alias>.pi`.
<!-- /pi-only -->
