...
4. **Fan-out (Phase 2):** resolve this round's dispatch units by calling
   `resolveFanoutGroups(config, gate, angles, { fullLabel })` (`@dev-loops/core/config`) over
   the current head's resolved angle set minus the plan's `carried` angles (the Phase 1.2
   subtraction — never `mustRerun`); `fullLabel` is whether the PR carries `gate:full`
   (Phase 1's `--full-label` input). In **grouped mode** (`gates.fanout.mode` unset or
   `grouped`, the shipped default, and no `gate:full`), angles map onto the static
   `gates.fanout.groups` table and the resolver returns one group per dispatch unit — an
   angle in no configured group is its own singleton group. In **per-angle mode**
   (`gates.fanout.mode: per-angle`) or under `gate:full`, every dispatch unit is a
   single-angle singleton group, reproducing today's one-reviewer-per-angle fan-out
   verbatim. Plan batches with `planFanoutBatches(groups.map((g) => g.name), cap)` (it
   batches by count, so group names stand in for the dispatch units) using the
   `gates.maxFanoutReviewers` cap (default 8); spawn one scoped `review` agent per
   returned group (plain Agent tool), parallel up to the cap, sequential for overflow
   (record any degradation in the gate evidence). Each reviewer follows [review agent
   scoped angle-review mode](../../agents/review.md): fresh context, read-only, briefed
   with every angle in its group (each angle's own prompt, all appended after the one
   shared invariant prefix — `GATE-EXEC-BRIEFING-PREFIX`), running the mandatory
   fresh-context guard ONCE with `--scope <gate>-group-<name>` (a per-angle dispatch keeps
   the existing `--scope <gate>-<angle>` naming, since its group name IS the angle name).
   The reviewer writes its per-angle findings artifact to
   `tmp/gate-reviews/<repo-slug>/pr-<N>/<gate>-<headSha>/<angle>.json` **once per angle in
   its group** — a 3-angle group therefore writes 3 artifacts, each with its own verdict
   and stamped with the reviewed head (`GATE-EXEC-ARTIFACT-HEAD-STAMP`) — never one merged
   artifact for the group. This keeps fan-in, the ledger, coverage checks, and the
   head-stamp guard byte-identical to per-angle mode, since all four read per-angle
   artifacts and never the reviewer that produced them. Grouped-round provenance records
   the group name on every angle a group's reviewer covered (`fanoutReviewerPairingError`'s
   within-group exception in [Gate Review Sub-Loop
   Contract](../docs/gate-review-sub-loop-contract.md#fan-out-provenance-closing-the-self-produced-artifact-loophole))
   — not restated here.
