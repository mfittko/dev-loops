...
`COPILOT-FOLLOWUP-WAIT-TOOLS`: The agent MUST wait only through allowed deterministic tools: `detect-copilot-loop-state.mjs` (one-shot), `scripts/github/probe-copilot-review.mjs --repo <owner/name> --pr <number> --timeout-ms <ms>` (the bounded FOREGROUND Copilot-review probe; `--timeout-ms 0` = a single immediate check), `dev-loops loop watch-cycle` (persistent), `copilot-pr-handoff.mjs --watch-status` (refresh after timeout/idle), `dev-loops loop watch-ci --repo <owner/name> --pr <number>` (provider-agnostic CI: CircleCI / Actions / external commit-status), `scripts/github/wait-pr-checks.mjs --repo <owner/name> --pr <number> --timeout <seconds>` (same provider-agnostic CI wait, with a direct 0/1/2 process-exit-code contract for shell/scripted callers instead of a JSON `status` field; `--timeout 0` = a single immediate check), `gh run watch <run-id> --repo <owner/name>` (Actions-only fallback when the run id is known); otherwise exit and resume later from a fresh detector call. Under the Claude Code harness the wait MUST run in the FOREGROUND (a bounded inline probe with an explicit `--timeout`/`--timeout-ms`; `0` = a single immediate check); a backgrounded `until`/`while … sleep … done` poll loop or bare-`&` backgrounding of a probe/wait script is forbidden and mechanically denied by the PreToolUse Bash-gate (`commandContainsDetachedWaitTool` / `decideBashGate`) for the coordinator and every subagent — Claude Code has no async wake, so a backgrounded wait is never joined and orphans past the stop.

<!-- rule: SUBAGENT-STOP-REAP -->
`SUBAGENT-STOP-REAP`: under the Claude Code harness, the `SubagentStop` hook (`.claude/hooks/subagent-stop-reaper.mjs`, deciding via `decideSubagentStopReap`) reaps the agent's own detached background job groups before allowing the stop — a safety net for anything that slipped past the `COPILOT-FOLLOWUP-WAIT-TOOLS` prevention. Its ownership boundary is verifiable and needs three conditions together, not two: it signals ONLY process-group LEADERS (a process whose own pgid equals its pid) that share the reaper's SESSION, excluding the reaper itself and the session leader, AND whose own command matches the wait/probe helper signature (`commandInvokesWaitProbeHelper`) — session + group-leader scoping alone is not sufficient ownership proof, since a foreground job-control job or an unrelated detached process (e.g. the UI-review server) can share the same session as a group leader. The command-signature match is what makes `process.kill(-pgid)` reach only a wait/probe job's own group and never that unrelated leader (mirrors the `ui-review-teardown` `process.kill(-pgid)` pattern). It never touches unrelated processes, other sessions, or git/worktree state (the uncommitted-work guard owns that). It fails closed on non-POSIX (win32 cannot signal a process group) and never blocks the stop. Prevention (the bare-`&` Bash-gate deny) is the root-cause fix; this reaper is the net.
