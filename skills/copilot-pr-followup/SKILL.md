...
- **Persona mapping:** resolve each angle's authoritative persona, focus prompt, and model with `dev-loops gate resolve-role --gate pre_approval_gate --angle <angle>`: on exit 0, use the returned persona and model, and use the returned `prompt` when present, otherwise review the angle by name; on a nonzero exit, stop and emit the blocked result via `emit-reviewer-blocked.mjs`. The fan-out emitter carries each resolved prompt in the unit's work order (`angleInstructions`) so the reviewer knows exactly what to look for.
<!-- pi-only -->
