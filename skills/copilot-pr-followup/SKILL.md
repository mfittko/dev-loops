...
- **Persona mapping:** each reviewer resolves its angle's persona and focus prompt by running the sanctioned CLI `dev-loops-run cli/index.mjs gate resolve-role --angle <name>` (which wraps `resolveReviewerRole` over the fully merged config, so it stays correct when a repo overrides an angle in `.devloops`). Use the returned `prompt` as each reviewer's focus so the reviewer knows exactly what to look for. When the CLI reports `fallback: true` or a null/empty `prompt` (`status` `fallback`/`prompt-missing`), that reviewer reviews the angle by name with no angle-specific focus instruction — never a grep of the shipped defaults, never a blocked reviewer.
<!-- pi-only -->
- **Model tier:** resolve each reviewer's model from the sanctioned CLI's primary `model` field by running `dev-loops-run cli/index.mjs gate resolve-role --angle <name> --harness pi`. That CLI resolves the tier with `kind: "angle"` semantics, which force the review (high) tier for the gate activity even when the angle name collides with a routine role (the `docs` angle resolves high, not the `docs` writer role's low tier). Pass the resolved model at dispatch **only when non-null** — a no-op on Pi until an operator sets `models.tiers.<alias>.pi`.
<!-- /pi-only -->
