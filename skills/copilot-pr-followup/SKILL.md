...
1. **Cache primer (Phase 1.5, optional):** when `gates.primeSharedPrefix` is true (default false), before fan-out send ONE angle-less primer over the byte-identical briefing prefix and **await it** (barrier), then release the fan-out over the same model + prefix — turning the cold-cache race into 1-write-N-reads (`GATE-EXEC-PRIME`). On a direct-API provider also verify `cache_write_tokens > 0` and pin the `prompt_cache_key`/breakpoint; under a subagent-tool harness those are unavailable so the primer is best-effort/unverifiable. Skip entirely when false.
