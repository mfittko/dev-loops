...
- `suppressed_post_convergence_docs_only`: a carried convergence under `COPILOT-STATE-CARRIED-CONVERGENCE` ([Copilot Loop State Graph](../docs/copilot-loop-state-graph.md)), at or below the round cap: no fresh blocking round was forced and the prior converged Copilot review still stands. Do NOT enter a Copilot wait seam; treat the PR as converged and proceed to `pre_approval_gate`, which `detect-pr-gate-coordination-state.mjs` allows for the same head. An unresolved thread or any code/test/config/CI or unclassifiable delta re-opens the round as normal
