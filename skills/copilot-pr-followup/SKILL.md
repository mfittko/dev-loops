...
2. **Carry-forward (Phase 1.2):** whenever this gate already recorded a findings-log at an EARLIER head, resolve which angles still need a fresh reviewer before dispatching any: `node <resolved-skill-scripts>/github/resolve-angle-carry-forward.mjs --repo <owner/name> --pr <number> --gate <gate> --prev-head <prior_head_sha> --head-sha <current_head_sha>`, run from the worktree checked out at the current head. Dispatch reviewers in Phase 2 for the returned `mustRerun` angles ONLY; each `carried` entry keeps its prior clean verdict and is recorded in Phase 3's provenance with that entry's `carriedFromHead` plus the prior reviewer identity it carries — never a fabricated fresh review. Skip this step on a gate's first round (no prior head). The rule, its fail-closed defaults, and the provenance shape are owned by `GATE-EXEC-ANGLE-CARRY-FORWARD` in [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md) — not restated here. The CLI fails closed in two layers. It refuses to emit a plan at all — stderr `{"ok":false,...}`, exit 1 — when the prior log is missing, unreadable, carries no per-angle provenance, records a non-hex head, or is not `clean`, and when the worktree is not checked out at `--head-sha`. Otherwise it emits a plan, and every angle it cannot prove safe lands in `mustRerun` with its reason (configured mandatory / always-include angle, a rename or copy anywhere in the delta, an unclassifiable file, a delta that rewrites the dev-loop config source, an empty delta, an angle with no declared review surface, or a delta touching the angle's surface). On a refusal, fan out every resolved angle exactly as if no prior head existed; never treat exit 1 as "nothing to re-run".
