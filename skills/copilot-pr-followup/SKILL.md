...
- `suppressed_post_convergence`: the default converged-once carry under `COPILOT-STATE-CARRIED-CONVERGENCE` ([Copilot Loop State Graph](../docs/copilot-loop-state-graph.md)), below the cap, at the cap, or under `--force-rerequest-review`: the latest Copilot review converged on an earlier head, so no fresh round was opened, whatever the delta. Do NOT enter a Copilot wait seam; proceed to `pre_approval_gate`, which `detect-pr-gate-coordination-state.mjs` allows for the same head. An unresolved thread or an outstanding request re-opens the round as normal
- `suppressed_post_convergence_docs_only`: the strict-mode carry (`refinement.requireCopilotConvergenceAtLatestHead: true`) or an operator suppression marker under `COPILOT-STATE-CARRIED-CONVERGENCE`, at or below the round cap: no fresh blocking round was forced and the prior converged Copilot review still stands. Do NOT enter a Copilot wait seam; treat the PR as converged and proceed to `pre_approval_gate`, which `detect-pr-gate-coordination-state.mjs` allows for the same head. In strict mode an unresolved thread or any code/test/config/CI or unclassifiable delta re-opens the round as normal
