...
- **Persona mapping:** each reviewer resolves its angle's persona and focus prompt by running the sanctioned CLI `dev-loops gate resolve-role --angle <name>` (which wraps `resolveReviewerRole` over the fully merged config, so it stays correct when a repo overrides an angle in `.devloops`). Use the returned `prompt` as each reviewer's focus so the reviewer knows exactly what to look for.
<!-- pi-only -->
