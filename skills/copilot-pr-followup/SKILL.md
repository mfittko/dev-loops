...
- `suppressed_post_convergence_docs_only`: returned at the round cap AND, since #2316, below it, whenever the post-convergence head bump was a provable pure doc/prose delta since the last Copilot-reviewed head OR an integrate-only base-move (base-relative reduction empties the delta), so no fresh blocking round was forced (the prior converged Copilot review still stands). Do NOT enter a Copilot wait seam; treat the PR as converged and proceed to `pre_approval_gate`. Any code/test/config/CI or unclassifiable delta re-opens the round as normal
