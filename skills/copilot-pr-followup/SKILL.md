...
- `suppressed_post_convergence`: the default converged-once carry under `COPILOT-STATE-CARRIED-CONVERGENCE` ([Copilot Loop State Graph](../docs/copilot-loop-state-graph.md)), below the cap, at the cap, or under `--force-rerequest-review`: the latest Copilot review converged on an earlier head, so no fresh round was opened, whatever the delta. An operator suppression marker in this mode also returns this status. Do NOT enter a Copilot wait seam; proceed to `pre_approval_gate`, which `detect-pr-gate-coordination-state.mjs` allows for the same head. An unresolved thread or an outstanding request re-opens the round as normal
- `suppressed_post_convergence_docs_only`: the strict-mode carry (`refinement.requireCopilotConvergenceAtLatestHead: true`), including an operator suppression marker in that mode, under `COPILOT-STATE-CARRIED-CONVERGENCE`, at or below the round cap: no fresh blocking round was forced and the prior converged Copilot review still stands. Do NOT enter a Copilot wait seam; treat the PR as converged and proceed to `pre_approval_gate`, which `detect-pr-gate-coordination-state.mjs` allows for the same head. In strict mode an unresolved thread or any code/test/config/CI or unclassifiable delta re-opens the round as normal
...
`COPILOT-FOLLOWUP-WAIT-TOOLS`: The agent MUST wait only through allowed deterministic tools: `detect-copilot-loop-state.mjs` (one-shot), `scripts/github/probe-copilot-review.mjs --repo <owner/name> --pr <number> --timeout-ms <ms>` (the bounded FOREGROUND Copilot-review probe; `--timeout-ms 0` = a single immediate check), `dev-loops loop watch-cycle` (persistent), `copilot-pr-handoff.mjs --watch-status` (refresh after timeout/idle), `dev-loops loop watch-ci --repo <owner/name> --pr <number>` (provider-agnostic CI: CircleCI / Actions / external commit-status; this CLI verb routes to `scripts/github/probe-ci-status.mjs`, which is equally sanctioned invoked directly with an explicit `--timeout-ms`), `scripts/github/wait-pr-checks.mjs --repo <owner/name> --pr <number> --timeout <seconds>` (same provider-agnostic CI wait, with a direct 0/1/2 process-exit-code contract for shell/scripted callers instead of a JSON `status` field; `--timeout 0` = a single immediate check), `gh run watch <run-id> --repo <owner/name>` (Actions-only fallback when the run id is known); otherwise exit and resume later from a fresh detector call. Under the Claude Code harness the wait MUST run in the FOREGROUND (a bounded inline probe with an explicit `--timeout`/`--timeout-ms`; `0` = a single immediate check); a backgrounded `until`/`while … sleep … done` poll loop or bare-`&` backgrounding of a probe/wait script is forbidden and mechanically denied — actor-independently and fail-closed, not bypassable by a wrapper (`timeout`/`nohup`/`sh -c`/a node loader flag) — by the PreToolUse Bash-gate (`commandContainsDetachedWaitTool` / `decideBashGate`) for the coordinator and every subagent — Claude Code has no async wake, so a backgrounded wait is never joined and orphans past the stop. This is prevention-only; a `SubagentStop` background-shell reaper safety-net for whatever slips past prevention is tracked as follow-up #2296.
...
Both gates run this same checkpoint review chain, owned end-to-end by [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md) (`GATE-EXEC-BUILD-ONCE-SEED`, `GATE-EXEC-BRIEFING-PREFIX`, `GATE-EXEC-SEPARATE-CHAINS`, `GATE-EXEC-POST-BEFORE-FIX`, `GATE-EXEC-REGATE-MANDATORY`, `GATE-EXEC-ANGLE-CARRY-FORWARD`, `GATE-EXEC-LIGHT-ESCALATION`); this section owns only this skill's dispatch of that chain. It is an **agent-orchestrated skill procedure** — a node script cannot spawn the per-angle reviewers, so a dedicated gate coordinator agent (`GATE-EXEC-GATE-COORDINATOR`) drives the fan-out and uses the pure `@dev-loops/core/loop/gate-fanin` helpers only for consolidation, batching, and ledger mapping.
...
2. **Carry-forward (Phase 1.2):** Skip this step ONLY on a gate's genuine first round (no prior findings-log at any head). Every re-gate (a prior findings-log exists at an earlier head) MUST run it — a non-clean prior round is never a reason to skip the resolver and re-fan everything; eligibility is decided per angle by the delta against each angle's review surface (`clean` OR `findings_present`). Before dispatch, run `node <resolved-skill-scripts>/github/resolve-angle-carry-forward.mjs --repo <owner/name> --pr <number> --gate <gate> --prev-head <prior_head_sha> --head-sha <current_head_sha> --spec-authority <identity-path>` from the current-head worktree. Use FULL 40-character SHAs; abbreviated keys cannot locate the prior log. `--prev-head` MUST be the most recently recorded head for this gate, which the dev-loop coordinator retains from the immediately preceding round in run state and passes to the gate coordinator, never guessed or discovered by scanning for a preferable older log. The resolver records its plan as the keyed `<gate>-<headSha>.carry-forward-plan.json` artifact at the current head; `GATE-EXEC-CARRY-FORWARD-PLAN-REQUIRED` makes the fan-out emitter refuse to spawn a reviewer on a re-gate head without it, so skipping this step fails closed rather than silently re-fanning.

   Follow `GATE-EXEC-ANGLE-CARRY-FORWARD` in the mandatory [owner](../docs/gate-review-sub-loop-contract.md#angle-carry-forward-fail-closed): `clean` and `findings_present` are eligible, subject to every surface, attribution and mandatory-angle guard. Carried open findings stay open and blocking as before; preserve their prior reviewer identity, `carriedVerdict` and `carriedFromHead`, never a fabricated fresh review. On success `{ carried, mustRerun }`, subtract, never substitute: dispatch every current-head resolved angle minus the plan's `carried` angles. `mustRerun` is informational, not the dispatch set; newly resolved angles absent from both lists still run. A non-zero exit is one of two shapes, distinguished by the plan artifact the resolver leaves at the current head: an ELIGIBILITY refusal (a readable prior log whose verdict is not carry-eligible) records a `{ ok: false, fallback: true }` marker, so rebuild+emit fans out every resolved angle (full re-dispatch); an OPERATIONAL or prior-log INTEGRITY failure (parse/usage error, no/mismatched/unreadable/corrupt prior log, wrong worktree) records NO marker, and the emitter then REFUSES the re-gate (`GATE-EXEC-CARRY-FORWARD-PLAN-REQUIRED`) — do not try to fan out around it, resolve the underlying failure (fix the `--prev-head`, the worktree, or the corrupt ledger) and re-run the resolver. Never treat exit 1 as "nothing to re-run" or search older logs to evade refusal.
3. **First-wave release:** follow `GATE-EXEC-FIRST-WAVE-RELEASE`. There is no primer and no lead reviewer: release the first emitted wave immediately after emission, bounded only by the emitted `maxConcurrent`. Cache priming is an optional adapter optimization and never a gate precondition.
4. **Fan-out (Phase 2):** read and follow the owner's [Phase 2](../docs/gate-review-sub-loop-contract.md#phase-2--fan-out-independent-reviewers-seeded-with-the-neutral-bundle), including `GATE-EXEC-FANOUT-DISPATCH-EMIT` and `GATE-EXEC-BRIEFING-PREFIX`. Rebuild the context after carry-forward with `--carried-angles <json> --prev-head <prior_head_sha>`, then run `node <resolved-skill-scripts>/github/emit-fanout-dispatch.mjs --repo <owner/name> --pr <n> --gate <gate> --head-sha <sha> --pending --carry-forward-plan <json>` using the resolver’s complete carry plan. This includes all-carried rounds: consume the keyed zero-unit plan in fan-in and ledger writing, without dispatching a reviewer. For full fallback (`carried: []`), rebuild with `--prev-head` alone and emit without `--pending`. Dispatch one collectable fresh-context `review` agent per emitted unit whose task is that unit's bounded work order (its `promptPath` bytes, relayed unchanged); the reviewer reads the referenced evidence itself. Obey the emitted `maxConcurrent`, not the unsplit context's wave plan. A unit whose angles share one non-`full` scope already names its `briefingVariants[scope]` file as a unit required read; never inline a variant or other evidence into the task. The reviewer runs its fresh-context guard once per unit, passing the emitted unit's `scope` verbatim as `--scope` — exactly as its prompt states, never composed from the unit name. Record actual reviewer identities and each emitted `group` for fan-in. Each reviewer writes one current-head artifact per angle, not one per group. Do not re-derive prompts, groups or personas in the conductor.
...
5. **Fan-in (Phase 3):** read and follow the owner's [Phase 3](../docs/gate-review-sub-loop-contract.md#phase-3--consolidation-fan-in-synthesis-and-disposition-ledger) and [disposition-ledger contract](../docs/gate-review-sub-loop-contract.md#disposition-ledger-and-durable-logging). Consolidate once:
   `dev-loops gate consolidate-fanin --repo <owner/name> --pr <n> --findings-dir <dir> --head-sha <current_head_sha> --gate <gate> --expected-dispatch-units <n> --out <findings-json-path> --ledger-out <ledger-path> --emit-plan <emit-plan-path> --jq '.severityCounts'`.
...
7. **Retry / fixer triage (Phase 5):** on EVERY round, including a clean verdict, apply the owner's Phase 4 and `GATE-EXEC-THREAD-DISPOSITION` to all findings. The judge controls relevance; fixer work receives only the `act` list and retains reproduction-based rejection. Within that list, triage high/medium/low defects and answer locatable question threads (answer, promote to a defect, or escalate); body-filed findings retain the owner's non-locatable disposition rules. Defects may be fixed cheaply in the same commit when already touching that code, otherwise deferred where permitted. Low findings may defer from round 1 — except a low the judge disposed `act`, which is a fix target declinable only on reproduction grounds (`GATE-EXEC-JUDGE-AUTHORITY-SPLIT`), never re-deferred on its severity label; the medium fix window is unchanged. Nits require no severity-axis fixer cycle; a judge `act` on a nit is the exception and reaches the fixer through the severity-blind act filter. Drive high/in-window-medium blocking findings through Step 7's fix → reply-with-resolving-commit → resolve loop.
...
- **Persona mapping:** the fan-out emitter resolves each assigned angle's persona and focus prompt and carries them in the unit's work order (`angleInstructions`, ADR 0086) — that carried work order is the fan-out reviewer's role source. A reviewer running standalone (outside a fan-out unit) or defensively re-validating a carried role never calls `resolveReviewerRole` inline; the sanctioned standalone/defensive role source is `dev-loops gate resolve-role --gate <draft_gate|pre_approval_gate|review|spike> --angle <angle>` (`--gate <operation>`, where `<operation>` is the review operation being run; `pre_approval_gate` is this gate's own case). Exit 0: use the returned persona, and use `prompt` when present, otherwise review the angle by name. On a nonzero exit: inside a fan-out unit (defensively re-validating a carried role), emit the blocked result via `emit-reviewer-blocked.mjs` as usual; running standalone (no findings directory or fan-out budget counters), instead stop that angle's pass and report it as blocked in the review verdict output.
<!-- pi-only -->
