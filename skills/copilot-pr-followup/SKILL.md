...
- **Persona mapping:** each reviewer resolves its angle's persona and focus prompt by running the sanctioned `gate resolve-role` CLI (`node <dev-loops-package-root>/cli/index.mjs gate resolve-role --angle <name>`, or `dev-loops-run cli/index.mjs gate resolve-role --angle <name>` under Claude; it wraps `resolveReviewerRole` over the fully merged config, so it stays correct when a repo overrides an angle in `.devloops`). When the CLI returns `ok: true` with `status: "resolved"`, continue normally and use the returned `prompt` as that reviewer's angle focus instruction. When it returns `ok: true` with `status: "fallback"` or `status: "prompt-missing"`, continue without treating the reviewer as blocked and review the angle by name with no angle-specific focus instruction — never a grep of the shipped defaults. For ANY other outcome (`ok: false`, or any other `status` such as `unresolved` for an unknown/typo angle or `config-error` for a broken config layer), STOP and emit the blocked-reviewer artifact (`scripts/github/emit-reviewer-blocked.mjs`) instead of proceeding with an untrusted role.
<!-- pi-only -->
- **Model tier:** resolve each reviewer's model from the sanctioned CLI's primary `model` field by running `node <dev-loops-package-root>/cli/index.mjs gate resolve-role --angle <name> --harness pi` (resolve the package root per [dev-loop SKILL](../dev-loop/SKILL.md)). That CLI resolves the tier with `kind: "angle"` semantics, which force the review (high) tier for the gate activity even when the angle name collides with a routine role (the `docs` angle resolves high, not the `docs` writer role's low tier). Pass the resolved model at dispatch **only when non-null** — a no-op on Pi until an operator sets `models.tiers.<alias>.pi`.
<!-- /pi-only -->
