...
5. **Verdict (Phase 4):** post via the [Gate comment command](#mandatory-gate-comment-command-contract) using `--execution-mode fanout_fanin` and `--findings-json <path>` built from the same per-angle artifacts consolidated in Phase 3 — that flag's accepted shapes are owned by the Gate comment command section above; do not restate them here. When Phase 3's consolidation result carries `commentBudgetExceeded: true` (the round was too large to render at minimum summary length, so `--findings-json` carries a budget-marked subset per angle), also pass `--findings-severity-counts` built from that same result's `severityCounts` field — it is always the true, unbudgeted totals, independent of what the rendered `--findings-json` block could fit.
