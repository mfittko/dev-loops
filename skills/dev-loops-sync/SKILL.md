...
name: dev-loops-sync
description: >-
  Provision or refresh the dev-loops agents and skills in a Multica workspace
  from a detected dev-loops source. Idempotent desired-state sync — imports the 7
  skills as SKILL.md-only, deletes deprecated carrier skills, pins agents to a
  runtime, sets DEVLOOPS_HOME, and binds skills to agents. Use when setting up
  dev-loops in a new Multica workspace or re-syncing after a dev-loops release.
compatibility: >-
  Targets Multica workspaces. Requires a Multica PAT (--pat, MULTICA_PAT, or a
  ~/.multica/profiles/*/config.json) and a reachable Multica API (--api or
  MULTICA_API). Node 20+, no dependencies.
...
```bash
node scripts/multica/dev-loops-sync.mjs \
