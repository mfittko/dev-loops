...
1. **Live source checkout (wins unconditionally — checkout wins by presence, no version comparison):** walk up from `cwd` (the mandatory worktree cwd) for a dir whose `package.json` `name === "dev-loops"` with a sibling `scripts/` dir; the first such ancestor is `<dev-loops-package-root>`, used unconditionally when found. This mirrors `.claude/bin/dev-loops-run`'s `isCheckout`/`findCheckout` so local-checkout precedence is uniform across harnesses: a live checkout wins even over a newer install under `~/.pi/agent/npm`, so a dogfooder's live edits always run and a stale user-level install never silently shadows the checkout. The walk-up is **bounded** to `cwd` ancestors and terminates at the filesystem root — never `find /`. Probe: `node -e 'const fs=require("node:fs"),p=require("node:path");const isCheckout=d=>{const j=p.join(d,"package.json");if(!fs.existsSync(j)||!fs.existsSync(p.join(d,"scripts")))return false;try{return JSON.parse(fs.readFileSync(j,"utf8")).name==="dev-loops"}catch{return false}};let d=process.cwd();for(;;){if(isCheckout(d)){console.log(d);process.exit(0)}const parent=p.dirname(d);if(parent===d)process.exit(1);d=parent}'` prints the checkout root and exits 0 on a hit; a non-zero exit means "no checkout ancestor, try the next candidate" (non-fatal). Only when no checkout is found does resolution fall through to the candidates below.
2. **Node module resolution** (best-effort, tried after the checkout walk-up): `node -e "try{const p=require('node:path');console.log(p.resolve(p.dirname(require.resolve('dev-loops/cli/index.mjs')),'..'))}catch{process.exit(1)}"` — resolves the package root when `dev-loops` is reachable from Node's module search path (notably under `~/.pi/agent/npm`); this is cwd-dependent and commonly misses from a target-repo cwd, so the probe is wrapped in try/catch (no stack trace, exits non-zero on miss) — treat a non-zero exit as "probe missed, try the next candidate", not a hard failure.
3. **Pi user-agent npm root** (reliable for user-level installs): `~/.pi/agent/npm/node_modules/dev-loops`.
4. **Package-relative (legacy):** `../../..` from this skill's own directory (the original package-local install layout).
5. **Global npm root:** `$(npm root -g)/dev-loops`.

NEVER fall back to `find /` or any unbounded filesystem walk to locate the CLI — it stalls and trips the needs-attention timeout. If every bounded candidate fails, stop and ask the orchestrator/operator for the dev-loops package root rather than searching. (The `dev-loop` agent resolves it analogously — same checkout-first walk-up, same "checkout wins, no version comparison" rule.)
<!-- /pi-only -->
...
4. **Use a dev-loops wrapper for `gh` reads — never an agent-level raw `gh`.** A raw `gh` call is a recorded advisory retro violation (issue #1077: reported, never blocking). If no script covers the read you need, treat it as a tooling gap: file/build a thin wrapper (reuse `scripts/lib/jq-output.mjs`, like the ones below), don't shell out. The reads/edits that already have wrappers:
   ...
   - Merge (the sanctioned merge path, orchestrator-owned) → `scripts/github/merge-pr.mjs --repo <o/r> --pr <n> --human-approved-by <login>` (`--method squash|merge|rebase`, `--stable-release`), **never raw `gh pr merge`**. It runs the full merge-precondition set fail-closed and refuses with a machine-readable reason naming the failing precondition; a verified `--human-approved-by <login>` is mandatory. See [Merge Preconditions](../docs/merge-preconditions.md#sanctioned-merge-wrapper-issue-1939).
...
**Gate fan-out dispatch discipline (bounded-parallel default, #1907):** gate fan-out is bounded-parallel by DEFAULT — up to `gates.fanout.maxConcurrent` dispatch units per wave, released as **ONE call per wave**, never N separate calls: `emit-wave-dispatch.mjs` emits a ready `subagent({ workflowScriptPath: "<wave>.js", cwd: <worktree>, async: false, timeoutMs: <bounded> })` per wave whose script body returns ONE `runs.all([...])` with a unique non-empty `key` per unit (`GATE-EXEC-FANOUT-DISPATCH-KEY`). "Blocking joins" means awaiting that one call before releasing the next wave — it does NOT mean one blocking `subagent` call per unit: under Pi the foreground guard rejects the second onward (`a subagent call is already in progress`), so per-unit calls silently serialize the round and burn the parent deadline. `tasks: [...]` is not an option in this pi-subagents version (it rejects it: `Legacy top-level chain and parallel inputs were removed; use workflowScript`) — never emit or document it. `gates.fanout.sequential: true` is the documented, RECORDED load fallback for a SIGTERM-prone environment (one unit per wave), never the default; the emitted wave fails closed on any other serialization rather than silently degrading a `gates.requireFanoutEvidence` gate, and a conductor that bypasses `emit-wave-dispatch.mjs` to compose its own per-unit calls is undetected at gate time. A transient dispatch failure (429/5xx) retries the same unit with exponential backoff; a hard 4xx (e.g. `402`) escalates to the supervisor/operator immediately instead of retrying into the same wall; provider choice stays a per-dispatch decision (`STICKY-PROVIDER-PIN`). Under the default one-reviewer-as-primer form, pass that unit's key as `emit-wave-dispatch.mjs --primer-key <key>` so the primer is emitted as its own first wave and never re-dispatched inside wave 1; a backoff-reduced batch uses the bounded `--max-concurrent <n>` (never above the emit-plan's recorded bound, recorded as an explicit degradation). Post-merge, the only remaining steps are the main-green check, one board-move attempt, and the final report — never re-running consolidation machinery whose artifacts already exist. Owning rules: `GATE-EXEC-FANOUT-WAVE-DISPATCH`, `GATE-EXEC-FANOUT-DISPATCH-KEY`, `GATE-EXEC-FANOUT-SEQUENTIAL-FALLBACK`, `GATE-EXEC-DISPATCH-RETRY-BACKOFF`, `GATE-EXEC-END-OF-RUN-CONTRACT` in [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md); `STICKY-PROVIDER-PIN` in [Anti-patterns](../docs/anti-patterns.md).
...
1. **Discover:** `node <dev-loops-package-root>/cli/index.mjs queue parked-unrefined --repo <owner/name>`
...
3. **Promote:** after `grill-clean` and confirmation by `detectIssueRefinementArtifact`,
   run `node <dev-loops-package-root>/cli/index.mjs queue move --repo <owner/name> --item <issueNumber> --to-column "<pickup column>"`
