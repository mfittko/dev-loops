...
1. Fetch comments into their own JSON file, extracting past the wrapper's envelope: for **tracker-first**, `node scripts/github/view-issue.mjs --repo <owner/repo> --issue <n> --json comments --jq '.issue.comments' > <comments-path>` (never raw `gh`); for **PR body**, the same with `scripts/github/view-pr.mjs ... --jq '.pr.comments' > <comments-path>`. The `--jq` extraction is required: the wrapper's own output is `{ "ok": true, "issue"|"pr": { "comments": [...] } }`, and the detector's `--comments-file` accepts only a bare array or `{ "comments": [...] }` — feeding it the unextracted envelope fails closed.
2. Run `node scripts/loop/detect-refinement-grill-state.mjs --body-file <body-path> --surface <issue|pr> --comments-file <comments-path>`.
