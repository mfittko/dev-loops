...
/**
 * Rewrite a bare `node scripts/...` invocation into the resolved-scripts-root placeholder form
 * (`node <resolved-skill-scripts>/...`) for the generated Claude tree (issue #2123). The Claude
 * plugin's `cwd` is never the dev-loops package root, so a bare cwd-relative `node scripts/...`
 * cannot resolve there; `<resolved-skill-scripts>` is resolved at runtime by the vendored
 * `resolve-scripts-root.mjs` (prefers a live checkout, else the bundled copy, else a hard-stop —
 * never a silent raw-`gh` fallback). Applied at GENERATION time only, mirroring
 * `rewriteCliInvocation`, so the canonical Pi/source docs (and everything that pins their exact
 * wording) are untouched — only the generated Claude tree is rewritten.
 *
 * Left UNCHANGED: a documented "source-repo fallback" note — e.g. `(source-repo fallback: \`node
 * scripts/...\`)`, or a "Detect the missing helper: try \`node scripts/...\`" probe — identified by
 * one of those two literal markers immediately BEFORE the match (a narrow lookbehind window, not a
 * broad nearby-word scan, so an unrelated "fallback" mention later in the same paragraph — e.g. "…
 * remains the manual fallback." two sentences on — does not spuriously exempt a genuine
 * invocation). Those are intentionally-bare illustrative examples of the cwd-relative source-repo
 * form, not a genuine invocation this tree runs.
 *
 * @param {string} body
 * @returns {string}
 */
export function rewriteBareScriptsInvocation(body) {
  const text = String(body);
  const pattern = /node\s+scripts\//g;
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    const before = text.slice(Math.max(0, start - LOOKBEHIND_WINDOW), start);
    const isDocumentedFallback = DOCUMENTED_FALLBACK_MARKERS.some((marker) => marker.test(before));
    result += text.slice(lastIndex, start);
    result += isDocumentedFallback ? match[0] : "node <resolved-skill-scripts>/";
    lastIndex = end;
  }
...
/**
 * Rewrite a bare `dev-loops <category> <subcommand>` invocation opening a FENCED code block line
 * into the version-pinned `npx dev-loops@<version> ...` form for the generated Claude tree (issue
 * #2123) — the same reasoning `rewriteCliInvocation` already applies to the package-local CLI
 * form, extended to the bare `dev-loops` citation shape some skills still invoke directly. Scoped
 * to fenced-code-block lines ONLY (never a mid-sentence prose mention of `dev-loops <verb>` as a
 * noun, and never the source docs — generation-time only, like `rewriteBareScriptsInvocation`
 * above), so prose and everything that pins the source's exact wording are untouched.
 *
 * @param {string} body
 * @param {string} version
 * @returns {string}
 */
export function rewriteBareDevLoopsInvocation(body, version) {
  let inFence = false;
  return String(body)
    .split("\n")
    .map((line) => {
      ...
      if (inFence && /^\s*dev-loops\s+[a-z]/.test(line)) {
        return line.replace("dev-loops", `npx dev-loops@${version}`);
      }
      return line;
    })
    .join("\n");
...
/** Compose the two generation-time-only scripts-root rewrites (issue #2123): a bare `node
 * scripts/...` wrapper invocation and a bare fenced `dev-loops <sub>` invocation, both routed for
 * the generated Claude tree only — the canonical Pi/source docs are never touched by either. */
function applyScriptsRootRewrites(body, version) {
  return rewriteBareDevLoopsInvocation(rewriteBareScriptsInvocation(body), version);
}
...
export function transformAgent({ source, raw, version = "latest", config = {} }) {
  ...
  const body = rewriteGeneratedRepoDocLinks(
    applyScriptsRootRewrites(rewriteCliInvocation(stripPiOnlyBlocks(rawBody), version), version),
  );
...
export function transformSkill({ source, raw, version = "latest" }) {
  const { frontmatter, body: rawBody } = splitFrontmatter(raw, source);
  const body = applyScriptsRootRewrites(rewriteCliInvocation(stripPiOnlyBlocks(rawBody), version), version);
