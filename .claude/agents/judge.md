...
- Coverage-expansion findings are dispositioned against [VALIDATE-COVERAGE-ADMISSION](../skills/docs/validation-policy.md) (canonical owner; do not restate it here): a coverage request that fails the admission rule is `reject`, so the act-list filter keeps it out of fixer dispatch, and a repeated coverage demand stays `reject` — checked against the prior-round ledgers you already hold — until new evidence names a distinct behavior or boundary risk. Demonstrated fail-open, security, acceptance-criterion, and public-contract defects keep their established disposition.
