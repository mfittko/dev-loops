...
Your dispatch already delivers those rules at point-of-action, so do NOT read the owner contracts in full as a precondition to reviewing. The composed briefing you receive (built by `scripts/github/emit-fanout-dispatch.mjs`'s composer core) carries the byte-identical invariant prefix — repo/PR/head/worktree, the gate-context artifact path, the mandatory `verify-fresh-review-context.mjs` isolation check, the worktree-absolute findings write-path, the source-read invariant, and the build-once diff + adjacent-code seed — followed by your dispatch unit's angle suffix. On the sanctioned one-shot dispatch path (`scripts/github/emit-fanout-dispatch.mjs`) that suffix only NAMES your angle(s): resolve each named angle's persona, focus prompt, and model tier by running the sanctioned `gate resolve-role` CLI (`npx dev-loops@1.0.4-pre.0 gate resolve-role --angle <name>`, or `dev-loops-run cli/index.mjs gate resolve-role --angle <name>` under Claude) — exactly as the full-PR default mode below does. When the CLI returns `ok: true` with `status: "resolved"`, continue normally and use the returned `prompt` as the angle's focus instruction. When it returns `ok: true` with `status: "fallback"` or `status: "prompt-missing"`, continue without a blocked reviewer and review the angle by name with no angle-specific focus instruction; never grep the shipped defaults. For ANY other outcome — `ok: false`, or any other `status` such as `unresolved` or `config-error` — STOP and emit the blocked-reviewer artifact via `scripts/github/emit-reviewer-blocked.mjs`. The coordinator never extracts or re-derives the role for you. That CLI reads the fully merged config and applies the repo's `.devloops` merge-by-name (it wraps the `resolveReviewerRole` function from `@dev-loops/core/config`), so it returns the correct persona/model even when a repo overrides an angle. You MUST NOT grep or read `packages/core/src/config/extension-defaults.yaml` directly to derive a persona/model: the shipped defaults miss the config-layer merge and yield the wrong role. A coordinator MAY instead author the persona text directly into the suffix; either way the adversarial behavior below is yours to apply. Act on that briefing. The references below name the single owner of each rule (for provenance, and for the exact anchor to consult only if one rule is unclear), never a whole-file read:
...
- The adversarial reviewing behavior is owned by `COPILOT-FOLLOWUP-ADVERSARIAL-BRIEFING` in the [Copilot PR Follow-up Skill](../skills/copilot-pr-followup/SKILL.md); apply it whether your suffix names the angle (you resolved the persona above via the CLI) or carries authored persona text: read the FULL diff (from `scope.diffPath`, or reconstruct it with `git diff` against the change base when `scope.diffPath` is null/missing — never a hunk-only review) plus the bundled adjacent code rather than re-deriving them, then hunt concrete `file:line` defects (edge cases, input validation, numeric coercion incl. NaN/Infinity/floats/negatives, null/undefined, boundary conditions, mismatched caller/callee contracts, dedup/identity bugs) over process nits, recording in the optional `contextWidened` field only the widening that moved your judgment (see the field definition below).
- Every dispatch unit is bounded (`REVIEWER_UNIT_BUDGET`, `@dev-loops/core/loop/reviewer-unit-bound`): at most 45 model turns and 50 tool calls, scoped to only the angle(s) named in your suffix, with polling PR/CI/Copilot state, network status probes, rerunning validation, and inspecting orchestration runtime internals all prohibited. The suffix names the exact numbers for the current build; on EITHER of the two ways a unit can fail its bound — you exceed that budget, or you cannot finish reviewing every assigned angle within it — never report clean: run `scripts/github/emit-reviewer-blocked.mjs` (invocation named in the suffix, built from values you already have: the briefing's head SHA and findings directory, your assigned angles, the angles you actually finished, and your consumed turns/tool-calls) to emit a durable `blocked` artifact naming the unreviewed angle(s) instead.
...
- Default pre-approval gate contract: before a review declares a branch/PR review-complete, approval-ready, merge-ready, or ready for final handoff, explicitly cover the review angles resolved from config (`resolveGateAngles(config, "preApproval")` from `@dev-loops/core/config`). For each angle, resolve the persona, prompt, and model tier by running the sanctioned `gate resolve-role` CLI (`npx dev-loops@1.0.4-pre.0 gate resolve-role --angle <name> --gate preApproval`, or `dev-loops-run cli/index.mjs gate resolve-role --angle <name> --gate preApproval` under Claude). That CLI reads the fully-merged config (applying the repo's `.devloops` merge-by-name over the shipped defaults; it wraps `resolveReviewerRole`), so do NOT grep or read `packages/core/src/config/extension-defaults.yaml` directly to derive a persona/model — the shipped defaults miss the config-layer merge and yield the wrong role. When the CLI returns `ok: true` with `status: "resolved"`, continue normally and use the returned `prompt` as the primary focus instruction for that review pass. When it returns `ok: true` with `status: "fallback"` (a configured angle with no dedicated persona) or `status: "prompt-missing"` (a resolved persona whose focus prompt is null/empty), continue without treating the reviewer as blocked and review the angle by name against the diff with no angle-specific focus instruction; do not fall back to grepping the shipped defaults. For ANY other outcome — `ok: false`, or any other `status` (e.g. `unresolved` for a typo/unknown angle, `config-error` for a broken config layer) — STOP and emit the blocked-reviewer artifact via `scripts/github/emit-reviewer-blocked.mjs` instead of proceeding with an untrusted role.
