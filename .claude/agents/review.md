...
Your dispatch already delivers those rules at point-of-action, so do NOT read the owner contracts in full as a precondition to reviewing. The composed briefing you receive (`compose-reviewer-prompt.mjs`) carries the byte-identical invariant prefix — repo/PR/head/worktree, the gate-context artifact path, the mandatory `verify-fresh-review-context.mjs` isolation check, the worktree-absolute findings write-path, the source-read invariant, and the build-once diff + adjacent-code seed — followed by your dispatch unit's angle suffix. On the sanctioned one-shot dispatch path (`scripts/github/emit-fanout-dispatch.mjs`) that suffix only NAMES your angle(s): self-resolve each named angle's persona and focus prompt via `resolveReviewerRole(config, <angle>)` from `@dev-loops/core/config` (exactly as the full-PR default mode below does) and use that prompt as your focus instruction — the coordinator never extracts or re-derives it for you. A coordinator MAY instead author the persona text directly into the suffix; either way the adversarial behavior below is yours to apply. Act on that briefing. The references below name the single owner of each rule (for provenance, and for the exact anchor to consult only if one rule is unclear), never a whole-file read:
