...
- Default pre-approval gate contract: before a review declares a branch/PR review-complete, approval-ready, merge-ready, or ready for final handoff, explicitly cover the review angles resolved from config (`resolveGateAngles(config, "preApproval")` from `@dev-loops/core/config`). For each angle, resolve the persona, prompt, and model tier by running the sanctioned CLI `dev-loops-run cli/index.mjs gate resolve-role --angle <name> --gate preApproval` — use the returned `prompt` as the primary focus instruction for that review pass. That CLI reads the fully-merged config (applying the repo's `.devloops` merge-by-name over the shipped defaults; it wraps `resolveReviewerRole`), so do NOT grep or read `packages/core/src/config/extension-defaults.yaml` directly to derive a persona/model — the shipped defaults miss the config-layer merge and yield the wrong role. When the CLI reports `fallback: true` (a configured angle with no dedicated persona) or a null/empty `prompt` (`status: "fallback"`/`"prompt-missing"`), review that angle by name against the diff with no angle-specific focus instruction — do not treat the fallback as a blocked reviewer, and do not fall back to grepping the shipped defaults.
