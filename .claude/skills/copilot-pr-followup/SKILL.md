...
7. **Retry / fixer triage (Phase 5):** on EVERY round, including a clean verdict, triage every gate-authored high/medium/low defect and answer every gate-authored question under the owner's Phase 4 and `GATE-EXEC-THREAD-DISPOSITION`. Defects may be fixed cheaply in the same commit when already touching that code, otherwise deferred where permitted; questions require an answer, promotion to a defect, or escalation as appropriate. Low findings may defer from round 1; the medium fix window is unchanged. Nits require no severity-axis fixer cycle; a judge `act` on a nit is the exception and reaches the fixer through the severity-blind act filter. Drive high/in-window-medium blocking findings through Step 7's fix → reply-with-resolving-commit → resolve loop.
   ...
   Then re-enter the chain at Phase 1 for the fixed head — Phase 1.2 decides what re-runs, including eligible findings-present angles whose open findings and provenance carry unchanged. Context-builder and fan-in always re-run. Repeat until the consolidated verdict is `clean` for the current head SHA AND 0 unresolved gate-authored threads remain (`GATE-EXEC-REGATE-MANDATORY`, `GATE-EXEC-THREAD-DISPOSITION`).
