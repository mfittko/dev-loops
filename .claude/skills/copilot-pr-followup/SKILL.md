...
4. **Fan-out (Phase 2):** turn the round's plan into per-unit reviewer prompts with the single sanctioned one-shot step `node <resolved-skill-scripts>/github/emit-fanout-dispatch.mjs --repo <owner/name> --pr <n> --gate <gate> --head-sha <sha> [--pending]` (`GATE-EXEC-FANOUT-DISPATCH-EMIT`, issue #2092). It emits `{ count, maxConcurrent, units: [{ scope, angles, group, promptPath }] }`, one unit per DISPATCH unit, and persists its round plan to the keyed `<gate>-<headSha>.emit-plan.json` sibling of the gate-context bundle — never a fixed-path stdout capture. Never re-derive persona or prompt text (the reviewer self-resolves via `resolveReviewerRole`), never spelunk `print-gates.mjs`, and never split a sanctioned auto-chunk bundle into singletons.
   - **Rebuild the context artifact first when Phase 1.2 ran**, so the briefing carries AC3's prior-round disposition memory (`resolvePriorDispositions`, issue #2175): with a non-empty carried subset, rebuild with `--carried-angles` AND `--prev-head <prior_head_sha>`, then pass `--pending` to the emitter; with `carried: []` (the full-fallback outcome), rebuild with `--prev-head <prior_head_sha>` alone and dispatch WITHOUT `--pending`. Omit `--pending` whenever no carry-forward subset exists. If every angle is carried, dispatch no emitter at all.
...
7. **Retry / fixer triage (Phase 5):** the fixer triages EVERY gate-authored defect finding (high, medium, AND low) and answers every gate-authored question on EVERY gate round. This is UNCONDITIONAL, never gated on "blocking findings" (#1585): on a clean verdict with open low/question threads the fixer STILL runs, receiving each gate-authored thread as a fix/triage/answer target — fix-if-cheap-in-the-same-commit when already touching that code, else defer, for a defect; an answer, promoting or escalating as needed, for a question.
   ...
   - The pass resolves every still-open DEFERRABLE gate-authored thread (low, nit, and out-of-window medium; it never fix-closes, and leaves high / question / in-window medium unresolved), FILES onto the tracked follow-up issue only the subset clearing the #1846 net-reduction bar (out-of-window medium always; low only when operator-visible; nit never), and REPORTS `unresolvedGateThreadCount`. The gate-close assertion (`fetchDraftGateEvidence` / `ready-for-review` / `pre-pr-ready-gate`) refuses to mark the gate satisfied while that count is non-zero. It posts nothing of its own and runs only the thread disposition pass (`GATE-EXEC-THREAD-DISPOSITION`). This is mandatory, not conditional on the verdict.
...
- **Board status sync (built-in, after ready-for-review):** the In-Progress board move runs automatically as a deterministic tail of `ready-for-review.mjs` (#1069), so no separate `sync-item-status` step is needed. Best-effort and NON-FATAL: local `gh` auth, exit 0 when the board is unconfigured, the item is absent, or the API fails, and never blocks marking the PR ready.
...
The post-merge hook also fast-forwards the main checkout's local `main` to `origin/main` (#1596) so read-only gate scripts run current code; the canonical step is [Merge Preconditions](../docs/merge-preconditions.md) "Post-merge". Best-effort and non-blocking (`--ff-only` refuses a diverged main without rewriting history).
