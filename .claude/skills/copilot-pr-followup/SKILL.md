...
2. **Cache primer (Phase 1.5, optional):** when `gates.primeSharedPrefix` is true (default false), before fan-out send ONE angle-less primer over the byte-identical briefing prefix and **await it** (barrier), then release the fan-out over the same model + prefix so reviewers cache-read it — turning the cold-cache race into 1-write-N-reads (`GATE-EXEC-PRIME`). No verification pass (agent harnesses own caching; nothing to read or pin). Skip entirely when false.
