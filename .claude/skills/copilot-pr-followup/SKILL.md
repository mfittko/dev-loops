...
and the canonical internal owner of the shared post-PR mechanics: PR discovery and interpretation,
async watch behavior, fix / reply-resolve / re-request flow, gate sequencing, final approval, and
...
> Under the Claude Code harness, run this loop **inline in a single agent**: the helper-owned wait tools block and return, so when `cycleDisposition` is `pending` and `terminal` is `false`, run the next watch cycle yourself — never exit on the wait boundary for a parent to re-dispatch. Keep driving until a terminal state or watch-budget expiry. Delegating a bounded fix to the `fixer` agent (Step 6) is task delegation, not re-dispatch, and stays allowed.

Max watch timeout: **30 minutes** (`policy-constants.mjs` COPILOT_REVIEW_WAIT_TIMEOUT_MS); expired budget + still `waiting_for_copilot_review` = hard stop. If the user explicitly asks for async handoff-only behavior, say that out loud and stop after the handoff boundary.
...
### Stale-installed-CLI: prefer worktree-source verdict/ledger tooling

Before running verdict/ledger tooling, resolve which layout it runs from:

```sh
node <resolved-skill-scripts>/loop/resolve-verdict-ledger-source.mjs --jq .preferredSource
```

- `worktree` → run `upsert-checkpoint-verdict.mjs`, `write-gate-findings-log.mjs`, `detect-checkpoint-evidence.mjs` and their gate helpers from the worktree/source `scripts/` layout.
- `installed` → run them from the resolved skill-scripts layout as usual.
- The helper fails soft to the installed layout when a version cannot be read, and never changes the gate path's own behavior.

### Source files under review vs. helper-script paths

The path rules above govern HELPER SCRIPT paths you RUN. They do **not** govern SKILL/DOC SOURCE FILES you REVIEW as content: read those from the worktree checkout under review (relative paths from the worktree cwd on the briefing prefix's `worktree:` line), never from an installed layout, and cross-check any line you cite in a finding against `git show HEAD:<path>` first. That rule, `GATE-EXEC-SOURCE-READ-WORKTREE`, and its briefing-prefix wiring are owned by [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md).
...
Practical rules: do not poll manually. `waiting_for_copilot_review` → `run-watch-cycle.mjs` or report-and-resume. `waiting_for_ci` with pending/none CI → `dev-loops-run cli/index.mjs loop watch-ci --repo <owner/name> --pr <number>` (provider-agnostic; covers CircleCI / Actions / external commit-status) or report-and-resume; `gh run watch <run-id>` is an Actions-only fallback. `dev-loops-run cli/index.mjs loop watch-cycle` also auto-routes a `waiting_for_ci` boundary to this CI watcher. The bounded zero-suite `crediblyGreen` exception is DERIVED by the detector from GitHub facts plus the previous-head rollup; no watch-route CLI flag asserts local validation (see [Wait / Watch Procedure](../docs/wait-watch-procedure.md)), so never infer green from missing CI. `ciStatus=failure` → stop/fix, never wait.
...
- the async-start contract is enforced in code: `outer-loop.mjs` fails closed without a visible async run id when `workflow.asyncStartMode: required` (relaxed automatically under the Claude Code harness)
...
1. inspect unresolved comments/threads and failing checks
   - the canonical loop re-entry read for the working set is `scripts/github/capture-review-threads.mjs --repo <owner/name> --pr <number> --unresolved --bodies`, which emits unresolved threads as `{ threadId, path, line, isOutdated, bodies }` with bodies already joined — no capture-then-parse step and no inline interpreter
   - enumerate unresolved threads with the thread/comment ids the reply-resolve helpers need via `scripts/github/list-review-threads.mjs --repo <owner/name> --pr <number> --unresolved-only`, never a hand-written `gh api graphql` query
...
3. classify findings:
   ...
   - question: never deferred; answer it, promoting to a defect severity if the answer reveals one, or escalating to the author when unanswerable. An unanswered question blocks gate-close exactly like an open defect
   - nit: cosmetic, non-defect; resolved-with-rationale immediately, no fixer action on the severity axis (judge-acted nits excepted), and NEVER filed to a tracked follow-up issue (net-reduction disposition policy)
...
11. after completing reply/resolve for a pass, verify zero unresolved threads remain via `dev-loops-run cli/index.mjs gate capture-threads` before proceeding
    ...
    - this thread-count check is necessary but not sufficient: `GATE-EXEC-FIXER-DISPOSITION-BOUNDARY` (in [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md#finding-threads-and-disposition)) additionally fails closed on a thread the fixer's own handoff marks tackled whose fixing commit is not contained by the observed PR head, or whose reply/resolve/re-verify step is incomplete. A zero unresolved-thread count does not waive it, and it forbids every gate-dispatch action — including requesting or re-requesting Copilot review — until `scripts/github/verify-fixer-disposition.mjs` reports the tackled set complete
12. <!-- rule: COPILOT-FOLLOWUP-ROUND-CAP --> `COPILOT-FOLLOWUP-ROUND-CAP`: The agent MUST decide whether another Copilot pass is desired, applying the round-cap/signal-gating rules below, only after GitHub-side reply/resolve work is done for the addressed threads and the refreshed thread snapshot proves zero unresolved threads remain
    ...
    - the inverse — a pure doc/prose post-convergence head bump must NOT force a fresh blocking Copilot round — is enforced in code: at the cap, `request-copilot-review.mjs` consults `resolveConvergenceCarryForward` (`@dev-loops/core/loop/gate-carry-forward`) on the delta since the last Copilot-reviewed head and returns `suppressed_post_convergence_docs_only` for a provable pure-doc bump, even under `--force-rerequest-review`. It is fail-closed: any code/test/config/CI or unclassifiable file, a rename/copy, a non-linear (rebased/amended) advance, or an unavailable compare re-opens the round
    - **below the cap**, that carry-forward proof is NOT applied automatically — a below-cap head-advanced bump normally gets a real Copilot pass. The one exception is an explicit, human-only operator withdrawal: see the "Head-advanced sibling case" in [Merge Preconditions](../docs/merge-preconditions.md). Never invoke it as an automated substitute for a real re-request
    - when the round limit is reached **and** the refreshed thread snapshot proves zero unresolved threads **and** current-head CI is a real green `success`, treat that clean state as eligible for `pre_approval_gate` fallback instead of deadlocking on another rerequest. `crediblyGreen` does NOT qualify: at the pre-approval/final boundary it fails closed to `BLOCKED_NEEDS_USER_DECISION` (unconfirmed CI), so the fallback waits for a real green
    ...
    - **Signal-gated re-request suppression:** `detect-copilot-loop-state.mjs` classifies review-thread comments by signal level. High-signal (bugs, security, contract violations) always re-requests; low-signal (cosmetic nits) never does, and when low-signal detection is enabled and its thresholds are met the machine returns a low-signal-converged terminal state routing to `pre_approval_gate`. Signal-level semantics are owned by [Copilot Loop Operations](../docs/copilot-loop-operations.md).
...
For a gate that ran via the fan-out/fan-in sub-loop, pass the structured per-angle review results via `--findings-json` (NOT the wall-of-text `--findings-summary`). That JSON file and the `severityCounts` substituted below both come from the SAME single fan-in call in the [Fan-in step](#gate-fan-outfan-in-procedure-agent-orchestrated) below, which consolidates the per-angle artifacts (`tmp/gate-reviews/<repo-slug>/pr-<N>/<gate>-<headSha>/<angle>.json`) into exactly the nested shape `--findings-json` accepts. The helper renders the per-angle breakdown and derives the single-line `**Findings summary:**` digest itself:
...
`--findings-ledger` is the durable findings log `write-gate-findings-log.mjs` reported as its output `path` in Phase 3 (`tmp/gate-findings/<owner-name>/pr-<N>/<gate>-<headSha>.json`) — NOT the flat `consolidate-fanin --ledger-out` array, which the helper rejects for carrying no repo/pr/gate/headSha identity. Pass it on every gate round: it makes the posted review the round's single finding surface, split by locatability (an in-diff finding becomes its own inline comment and the body carries only an aggregate `**Inline findings:**` count/pointer line; every other finding renders in full as a plain bulleted body item, stamped with an invisible fingerprint+disposition marker for cross-round tracking). Omitting it posts a bare verdict with no finding surface and no finding threads.

Substitute `<consolidate-fanin severityCounts>` with the fan-in CLI's own `severityCounts` field (its true, unbudgeted totals) — never a literal all-zero placeholder, and never a hand-typed value for a `findings_present`/`blocked` round. It can only RAISE the posted `**Findings summary:**` total above `--findings-json`'s own count, never lower it.

`--findings-json` accepts the per-angle review-results array (`[{ angle, verdict?, findings:[{severity, summary, file?, line?, disposition?}] }]`, the primary shape feeding `consolidateFanin`) and the flat per-finding array `consolidateFanin`/`toFindingsLogShape` produce (`[{ severity, summary, angle?, ... }]`), which it groups by each finding's `.angle`. A non-empty input matching neither shape is rejected rather than rendering all-clean. Fall back to `--findings-summary "<summary>"` when structured results do not exist (an inline run). A withheld `fanout_fanin` round (too many angles to render even hard-truncated) has neither shape: `--out` was never written, and the unbudgeted `--ledger-out` flat file MUST NOT be substituted for it. Post that round with `--findings-summary "<summary>"` PLUS the mandatory `--findings-ledger <findings-log-path>` — never `--findings-summary` alone, because for a gate with mandatory angles `upsert-checkpoint-verdict.mjs` proves coverage from that ledger's provenance and refuses the post when both flags are omitted ([Phase 3 — Consolidation](../docs/gate-review-sub-loop-contract.md#phase-3--consolidation-fan-in-synthesis-and-disposition-ledger) owns the full artifact/coverage rule).
...
`--execution-mode <fanout_fanin|inline_single_agent>` records how the gate inspection ran. Always pass it explicitly — a bare call with neither flag fails with an argument error. When the gate did not run via the fan-out/fan-in sub-loop ([Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md)), you MUST pass `--execution-mode inline_single_agent --inline-reason "<why>"`; inline mode requires a non-empty reason. The recorded `executionMode` is surfaced by `detect-checkpoint-evidence.mjs` and gated by `gates.requireFanoutEvidence`.
...
Reuse `check-size-budget.mjs` verbatim; never recompute its `computeSizeBudget`/`evaluatePrSizeBudget` logic by hand. Passing `--size-budget-json` explicitly stays the preferred path. Omitting it on a `pre_approval_gate` verdict makes `upsert-checkpoint-verdict.mjs` auto-derive the budget against the PR's base ref, or fail closed naming `--size-budget-json` as the escape hatch. `draft_gate` and `review` verdicts never auto-derive and post `null` size fields when the flag is omitted, which has no merge-gate effect: the size-budget merge gate reads `pre_approval_gate` evidence only.

### Gate fan-out/fan-in procedure (agent-orchestrated)

Both gates run the same checkpoint review chain, owned end-to-end by [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md) (`GATE-EXEC-BUILD-ONCE-SEED`, `GATE-EXEC-BRIEFING-PREFIX`, `GATE-EXEC-SEPARATE-CHAINS`, `GATE-EXEC-POST-BEFORE-FIX`, `GATE-EXEC-REGATE-MANDATORY`, `GATE-EXEC-ANGLE-CARRY-FORWARD`, `GATE-EXEC-LIGHT-ESCALATION`). Read that contract before running the chain; it is MANDATORY at this point of use, and this section owns only this skill's dispatch of it. It is an **agent-orchestrated skill procedure** — a node script cannot spawn the per-angle reviewers, so the conductor agent drives the fan-out and uses the pure `@dev-loops/core/loop/gate-fanin` helpers only for consolidation, batching, and ledger mapping.

1. **Context (Phase 1):**
   - Run `node <resolved-skill-scripts>/loop/run-gate-validation.mjs --repo <owner/name> --pr <number> --gate <gate> --head-sha <current_head_sha>` ONCE for this round (`GATE-EXEC-VALIDATION-ARTIFACT`).
   - Build the gate-context artifact via `scripts/github/write-gate-context.mjs`, passing `--validation-results <that artifact path>` and, when the PR carries the `gate:full` label, `--full-label`.
   - Consume `tmp/gate-context/<repo-slug>/pr-<N>/<gate>-<headSha>.json`: its FULL diff at `scope.diffPath` plus the top-level `adjacentCode` bundle are the build-once seed every reviewer uses verbatim. Do not re-derive them per reviewer.
   - Do NOT thread `--pr-body`/`--issue-body`/`--acceptance-criteria`: the CLI resolves the spec-of-record (PR body, every closing issue reference, each of those issues' bodies) itself and fails closed with no artifact rather than claiming the PR has no description. Pass them only to override the live values.
2. **Carry-forward (Phase 1.2):** whenever this gate already recorded a findings-log at an earlier head, resolve which angles still need a fresh reviewer BEFORE dispatching any. Skip this step on a gate's first round (no prior head).
   - Run `node <resolved-skill-scripts>/github/resolve-angle-carry-forward.mjs --repo <owner/name> --pr <number> --gate <gate> --prev-head <prior_head_sha> --head-sha <current_head_sha>` from the worktree checked out at the current head.
   - Pass both SHAs in FULL 40-character form: the log filename is keyed by the full SHA, so an abbreviated `--prev-head` finds no log and refuses forever.
   - `--prev-head` MUST be this gate's **most recently recorded** head — the exact head this same conductor wrote a findings-log for on the immediately preceding round (Phase 3's last write, or Phase 5's fixed head one iteration back), carried in the conductor's own run state. Never scan `tmp/gate-findings/` for it, and never reach past it to an older log.
   - On success the CLI emits `{ carried, mustRerun }`. Phase 2 dispatches **every angle Phase 1 resolved at the current head, minus the plan's `carried` angles** — subtract, never substitute, because the plan's universe is the PRIOR head's angle set and an angle first resolved at this head appears in neither list. `mustRerun` is informational only; never dispatch by consulting it.
   - On ANY non-zero exit (a `{"ok":false,...}` refusal or a usage error), fan out every resolved angle exactly as if no prior head existed. Never treat exit 1 as "nothing to re-run".
   - Refusal conditions, the per-angle must-re-run reasons, and the carried-entry provenance shape (`carriedFromHead` plus the prior reviewer identity, never a fabricated fresh review) are owned by `GATE-EXEC-ANGLE-CARRY-FORWARD` in [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md#angle-carry-forward-fail-closed).
3. **Cache primer (Phase 1.5, MANDATORY):** every fan-out primes the shared prefix (`GATE-EXEC-PRIME`). Default form = **one-reviewer-as-primer**: dispatch one reviewer first and release the rest once its prefix write has landed — on its first streamed token if the harness streams, else on its completion, never on an unobservable "start". A dedicated angle-less primer is the alternative. No verification pass.
4. **Fan-out (Phase 2):** turn the round's plan into per-unit reviewer prompts with the single sanctioned one-shot step `node <resolved-skill-scripts>/github/emit-fanout-dispatch.mjs --repo <owner/name> --pr <n> --gate <gate> --head-sha <sha> [--pending]` (`GATE-EXEC-FANOUT-DISPATCH-EMIT`). It emits `{ count, maxConcurrent, units: [{ scope, angles, group, promptPath }] }`, one unit per DISPATCH unit, and persists its round plan to the keyed `<gate>-<headSha>.emit-plan.json` sibling of the gate-context bundle — never a fixed-path stdout capture. Never re-derive persona or prompt text (the reviewer self-resolves via `resolveReviewerRole`), never spelunk `print-gates.mjs`, and never split a sanctioned auto-chunk bundle into singletons.
   - **Rebuild the context artifact first when Phase 1.2 ran**, so the briefing carries AC3's prior-round disposition memory (`resolvePriorDispositions`): with a non-empty carried subset, rebuild with `--carried-angles` AND `--prev-head <prior_head_sha>`, then pass `--pending` to the emitter; with `carried: []` (the full-fallback outcome), rebuild with `--prev-head <prior_head_sha>` alone and dispatch WITHOUT `--pending`. Omit `--pending` whenever no carry-forward subset exists. If every angle is carried, dispatch no emitter at all.
   - Spawn one scoped `review` agent per emitted unit (plain Agent tool) seeded with that unit's `promptPath` bytes verbatim, and record that unit's `group` on Phase 3's `--provenance`.
   - Wave the EMITTED units at most `maxConcurrent` at a time (the emitter's own output field), awaiting a free slot before launching the next. Do NOT bound this step by `artifact.fanout.wavePlan` — that plan counts unsplit units and would over-dispatch a wave slot. Record any degradation in the gate evidence.
   - When every angle in a unit declares the SAME non-`full` scope, ALSO seed that reviewer with the matching `briefingVariants[scope]` file recorded on the context artifact, layered AFTER the invariant prefix as an additional narrow read, never a replacement. A mixed-scope or `full` unit is seeded with the invariant prefix alone.
   - Each reviewer follows [review agent scoped angle-review mode](../../agents/review.md): fresh context, read-only, briefed with every angle in its unit.
   <!-- rule: COPILOT-FOLLOWUP-ADVERSARIAL-BRIEFING -->
   `COPILOT-FOLLOWUP-ADVERSARIAL-BRIEFING`: Each reviewer MUST be briefed to review like an external code reviewer hunting real bugs: read the FULL diff (from `scope.diffPath`, or `git diff` when null) and the bundled adjacent code (callers, callees, imports) rather than re-deriving them, then review adversarially for concrete defects (edge cases, input validation, numeric coercion incl. NaN/Infinity/floats/negatives, null/undefined, boundary conditions, mismatched caller/callee contracts, dedup/identity bugs) with `file:line` + the failing scenario — not process nits like "no test exists". Reviewers MAY widen scope (open adjacent repo files beyond the bundle) only when their angle genuinely needs more, recording in the optional `contextWidened` field only the widening that moved their judgment. This angle-specific text is what the conductor supplies as `scripts/github/compose-reviewer-prompt.mjs --angle-suffix-file` — the SANCTIONED way this briefing is assembled into one reviewer prompt, never a hand-assembled per-group preamble. Its LAYOUT (invariant block first, this adversarial prompt last) and the `--prefix-hash`/`--prefix-file` sentinel recording are owned by `GATE-EXEC-BRIEFING-PREFIX`.
   - Each reviewer's briefing also carries a known-findings block, appended AFTER this angle-specific prompt and never into the byte-identical prefix `GATE-EXEC-BRIEFING-PREFIX` hashes, listing every currently open or resolved finding thread regardless of author. Build it from `dev-loops-run scripts/github/capture-review-threads.mjs --repo <owner/name> --pr <number>` output (the full-bodies read, not `list-review-threads.mjs`'s 200-char excerpt), never an ad-hoc GraphQL call. Its content and dedupe contract are owned by `GATE-EXEC-FINDING-THREADS` in [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md#finding-threads-and-disposition).
5. **Fan-in (Phase 3):** consolidate via ONE call to the sanctioned fan-in CLI (`scripts/loop/consolidate-fanin.mjs`), which runs the briefing-prefix and dispatch-prompt verifiers itself before consolidating:
   ...
   dev-loops-run cli/index.mjs gate consolidate-fanin --findings-dir <dir> --head-sha <current_head_sha> --gate <gate> \
     ...
     --emit-plan <emit-plan-path> --jq '.severityCounts'
   ```

   - `--expected-dispatch-units <n>` is the number of dispatch units Phase 2 actually spawned reviewers for — the emitter's own `count` field. `fanout.pendingGroups.length` is only an upper bound on that (it counts unsplit units, and a partially-carried unit stays in it whole), so never pass it unreconciled; NEVER pass `fanout.wavePlan.length` (the WAVE count) or the per-angle artifact count (which false-fails every grouped round). When no reviewer was dispatched (an all-carried round), OMIT the flag entirely rather than passing `0`, which the CLI rejects.
   - `--head-sha` activates the `GATE-EXEC-ARTIFACT-HEAD-STAMP` stale-artifact guard; pass it on every round. `--emit-plan <path>` passes the emitter's keyed artifact for the `GATE-EXEC-EMIT-PLAN-KEY` round-key guard; pass it on every round that ran the emitter.
   - That ONE invocation writes `--out`/`--ledger-out` AND prints `.severityCounts`. Capture it (`severity_counts=$(... --jq '.severityCounts')`) for the verdict call below; never re-invoke the CLI to re-extract a value it already returned, and never hand-author or mutate its output with an inline interpreter.
   - When Phase 1.2 returned a plan, pass BOTH `--carried-angles <json>` (that plan's `carried` angle names) AND `--carry-forward-plan <json>` (the plan itself, or at least its `carried` array) — required together. Without them a carried angle is invisible to the coverage checks and the posted verdict reads as a truncated fan-out. Omit both on a gate's first round. The CLI treats `--carried-angles` as proof-carrying and fails closed per the sub-loop contract's refusal list.
   - Then write the disposition ledger BEFORE the visible verdict review (`GATE-EXEC-DISPOSITION-LEDGER`, `GATE-EXEC-POST-BEFORE-FIX`): `write-gate-findings-log.mjs --findings-file <ledger-path> --emit-plan <emit-plan-path> --provenance '<json>' --execution-mode fanout_fanin`, passing the CLI's `--ledger-out` file straight through. On a `gate:full` round also pass `--full-label` here (mirroring Phase 1) so its `resolvedGroups` cross-check resolves the same untiered table Phase 2 dispatched against.
   - `--provenance` is a SEPARATE flag on THIS write call and does not exist on `post-gate-findings.mjs` below. Omitting it drops every `carriedFromHead` attribution, which silently disables the NEXT round's carry-forward; declaring `--execution-mode fanout_fanin` makes the omission a hard write-time error for a gate with mandatory angles. Its shape is `{ distinctReviewers: <int>, perAngle: [...] }`: `distinctReviewers` is required, and `perAngle` MUST cover every resolved angle — a fresh entry per angle Phase 2 dispatched, plus one entry per Phase 1.2 `carried` angle carrying that entry's `carriedFromHead` and EVERY prior reviewer identity field it has (`reviewer`/`dispatchId`/`model`), never a hand-picked subset. A fresh entry from a GROUPED unit also carries `group` set to that unit's name, repeated identically on every fresh entry the same reviewer covers — you (the conductor) write it, since a reviewer's findings artifact has none. The plan's `reason` field is not part of the accepted shape.
   - The round's findings ride the Phase 4 verdict review, so no separate findings comment is posted. Only when a repo has explicitly opted into `gates.postFindingsComments` (default false), also post the same ledger via `node <resolved-skill-scripts>/github/post-gate-findings.mjs --repo <owner/name> --pr <number> --gate <gate> --head-sha <current_head_sha> --findings-file <ledger-path>` — no `--provenance` flag on this call.
6. **Verdict (Phase 4):** check whether the fan-in's `--out` file exists before posting. A withheld round (see the [Gate comment command](#mandatory-gate-comment-command-contract)'s withheld-round note) deleted it and has no `--findings-json` shape: post that round with `--findings-summary "<summary>"` PLUS `--findings-ledger <findings-log-path>`, never `--findings-summary` alone. Otherwise post via the [Gate comment command](#mandatory-gate-comment-command-contract) with `--execution-mode fanout_fanin` and `--findings-json <path>`, passing the same durable log to `--findings-ledger` so the findings ride the one posted review (`GATE-EXEC-FINDING-THREADS`). Do NOT run the disposition pass yet: `close-gate-findings.mjs` runs AFTER the Phase 5 fixer triage, so the fixer sees every gate-authored finding — including low ones — before the pass defer-closes them.
7. **Retry / fixer triage (Phase 5):** the fixer triages EVERY gate-authored defect finding (high, medium, AND low) and answers every gate-authored question on EVERY gate round. This is UNCONDITIONAL, never gated on "blocking findings": on a clean verdict with open low/question threads the fixer STILL runs, receiving each gate-authored thread as a fix/triage/answer target — fix-if-cheap-in-the-same-commit when already touching that code, else defer, for a defect; an answer, promoting or escalating as needed, for a question.
   - A nit thread is owed no fixer triage cycle on the severity axis (the one exception: a judge `act` on a nit reaches the fixer through judge-pass's severity-blind act filter). The disposition pass resolves a still-unresolved nit immediately, but NEVER files it to the tracked follow-up issue and NEVER stamps `disposition=deferred` on it (net-reduction disposition policy).
   - Defer is permitted from round 1 on for low findings (no forced fix window; the medium window is unaffected). Whether a fixer-deferred low is ALSO filed and stamped follows the same net-reduction gate: only when its own marker carries the finding's explicit `operatorVisible: true` signal. The conservative default (absent/false) is resolved-with-rationale, never filed.
   - Drive blocking findings (high, in-window medium) through the SAME fix → reply-with-resolving-commit → resolve loop used for external Copilot review comments (Step 7 above).
   - Then, at every gate close (clean or not) and AFTER the fixer triage, run `node <resolved-skill-scripts>/github/close-gate-findings.mjs --ledger <findings-log-path> --allowed-refs <governing-issue>` against that same path. `<governing-issue>` is the PR's governing (closing) issue resolved DETERMINISTICALLY — the `closingIssuesReferences` / `Closes #N` the Phase 1 gate-context bundle already resolved, passed as the CSV of all closing issue numbers for an umbrella PR, NEVER a hardcoded literal; omit the flag only when the PR closes no issue. This opens the bare-`#N` comment-id guard ONLY for that governing issue, so a bare reference to any UNrelated issue stays fail-closed — a scoped allowlist, not a blanket bypass.
   - The pass resolves every still-open DEFERRABLE gate-authored thread (low, nit, and out-of-window medium; it never fix-closes, and leaves high / question / in-window medium unresolved), FILES onto the tracked follow-up issue only the subset clearing the net-reduction bar (out-of-window medium always; low only when operator-visible; nit never), and REPORTS `unresolvedGateThreadCount`. The gate-close assertion (`fetchDraftGateEvidence` / `ready-for-review` / `pre-pr-ready-gate`) refuses to mark the gate satisfied while that count is non-zero. It posts nothing of its own and runs only the thread disposition pass (`GATE-EXEC-THREAD-DISPOSITION`). This is mandatory, not conditional on the verdict.
   - Then re-enter the chain at Phase 1 for the fixed head; Phase 1.2 decides what re-runs. Context-builder and fan-in always re-run. Repeat until the consolidated verdict is `clean` for the current head SHA AND 0 unresolved gate-authored threads remain (`GATE-EXEC-REGATE-MANDATORY`, `GATE-EXEC-THREAD-DISPOSITION`).
...
- **Board status sync (built-in, after ready-for-review):** the In-Progress board move runs automatically as a deterministic tail of `ready-for-review.mjs`, so no separate `sync-item-status` step is needed. Best-effort and NON-FATAL: local `gh` auth, exit 0 when the board is unconfigured, the item is absent, or the API fails, and never blocks marking the PR ready.
...
- **Required PR comment:** post a visible checkpoint verdict comment using the mandatory [Gate comment command](#mandatory-gate-comment-command-contract). Its field content and validation-reporting format (`GATE-COMMENT-VALIDATION-REPORTING`), the draft-boundary comment requirement (`GATE-COMMENT-DRAFT-REQUIREMENTS`) and posting-failure fail-closed behavior (`GATE-COMMENT-FAIL-CLOSED`) are owned by [Gate Review Comment Contract](../docs/gate-review-comment-contract.md).
...
- **Required PR comment:** post a visible checkpoint verdict comment using the mandatory [Gate comment command](#mandatory-gate-comment-command-contract). Its field content and validation-reporting format (`GATE-COMMENT-VALIDATION-REPORTING`), the pre-approval-boundary comment requirement (`GATE-COMMENT-PREAPPROVAL-REQUIREMENTS`) and posting-failure fail-closed behavior (`GATE-COMMENT-FAIL-CLOSED`) are owned by [Gate Review Comment Contract](../docs/gate-review-comment-contract.md).
...
The pre-merge gate evidence check fails closed on the PR's runner-coordination claim (`.pi/runner-coordination/<owner>/<name>/pr-<n>.json`): a fresh merge re-dispatch (new run id) is refused with `ownership_lost`, or `stale_runner` once the claim ages past the max-age window. A claim is normally cleared for you: the auto-loop releases it best-effort at any terminal stop (including the human approval checkpoint), the headless driver releases on process death, and `copilot-pr-handoff` acquires with `supersedeStale: true` so pre-flight handoff supersedes a confirmed-dead or stale claim. Each release is non-fatal and never clears a genuinely active competing run's claim, so one-runner-per-PR holds.

**Anti-trap — a fresh claim heartbeat is not proof of a live driver.** A completed/control run that claimed at takeover and ended without releasing still reads as a fresh heartbeat. LIVE requires a subagent run verified via `subagent status` (a harness command, not a repo script or CLI) showing an actively-updating `EXECUTING` child. Confirm real execution that way before ANY stand-down or dispatch decision; never trust the heartbeat alone.

If a stale claim still blocks the merge because the completing run could not release (crash or killed process), the sanctioned recovery for a lock held by a COMPLETED run is an explicit takeover by the merge run:
...
After a merge succeeds (or an explicit retrospective skip is authorized), write the durable retrospective checkpoint before exiting the subagent session. The retrospective itself MUST be a fresh-context, independent pass over the cycle's full agent/subagent tool-call record, dispatched like a gate reviewer and never self-authored inline by the working session (`RETRO-FRESH-CONTEXT-MANDATORY` in [Retrospective Checkpoint Contract](../docs/retrospective-checkpoint-contract.md)). Carrying `--repo`/`--pr`/`--merge-commit` is MUST, not optional — the next loop derives the retrospective gate from git ancestry between the recorded `identity.mergeCommit` and the base branch, so an identity-less record is treated as stale and the CLI rejects `complete`/`skipped` without it. Read the full merge commit oid with `node <resolved-skill-scripts>/github/view-pr.mjs --repo <owner/name> --pr <n> --json mergeCommit --jq .pr.mergeCommit.oid`. A `complete` record MUST also carry fresh-context provenance (`--retro-context fresh --record-source <path-to-tool-call-record>`; `inline` is rejected outright):
...
`queue sync-status --logical-column done` moves the merged item's board Status to the configured Done column; the logical name resolves through `queue.statusColumns`, so a renamed Done column still converges. Omit `--item` when the merged PR is itself the queue item — an unfilled/empty `--item` falls back to `--pr` rather than failing. Run it from the MAIN CHECKOUT and BEFORE any worktree removal: it resolves `.devloops` relative to `cwd` and has no `--repo-root` flag. Board and threshold come from `.devloops` (`tracker.board`, `queue.archiveOlderThanDays`, default 7d) using local `gh` auth — no CI, cron, or PAT.

Both steps are best-effort and NON-FATAL, with different exit contracts: `queue sync-status` exits 0 on any parsed invocation including board/API failures (its stdout JSON describes the skip), non-zero only on a usage error (1), an invalid `--jq` filter (2), or a falsy `--silent` predicate. `archive-done-items.mjs` exits non-zero on a usage error (1), a GitHub API error or invalid `--jq` filter (2), or project-not-found (3), so its `|| true` is load-bearing and masks those deliberately. Neither step blocks the merge or the retrospective.

`dev-loops-run cli/index.mjs queue reconcile` (idempotent, best-effort at loop startup) is the fallback convergence path when this sync is skipped or missed: it re-derives every item's column from live GitHub state.

The post-merge hook also fast-forwards the main checkout's local `main` to `origin/main` so read-only gate scripts run current code; the canonical step is [Merge Preconditions](../docs/merge-preconditions.md) "Post-merge". Best-effort and non-blocking (`--ff-only` refuses a diverged main without rewriting history).
