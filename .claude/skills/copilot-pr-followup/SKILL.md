...
1. **Cache primer (Phase 1.5, optional; direct-API only):** when `gates.primeSharedPrefix` is true (default false — keep false under a subagent-tool harness like Claude Code), before fan-out send ONE angle-less primer over the byte-identical briefing prefix, await it, verify `cache_write_tokens > 0`, then release the fan-out pinned to the same model / `prompt_cache_key` / prefix / breakpoint — turning the cold-cache race into 1-write-N-reads (`GATE-EXEC-PRIME`). Skip entirely when false.
