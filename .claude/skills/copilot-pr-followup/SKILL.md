...
`--findings-ledger` is the durable findings log `write-gate-findings-log.mjs` reported as its output `path` in Phase 3 (`tmp/gate-findings/<owner-name>/pr-<N>/<gate>-<headSha>.json`) — NOT the flat `consolidate-fanin --ledger-out` array, which the helper rejects because it carries no repo/pr/gate/headSha identity. Pass it on every gate round: it turns the posted review into the round's single finding surface, split into two tracks by locatability — an in-diff finding becomes its own inline comment, with the body carrying only an aggregate `**Inline findings:**` count/pointer line (never a per-finding row); every other finding renders in full as its own plain bulleted list item in the body (never a table) and is additionally stamped with an invisible fingerprint+disposition marker for cross-round suppression/deferral tracking. Omitting `--findings-ledger` posts a bare verdict body with no finding surface and no finding threads at all.
...
`--findings-json` accepts the per-angle review-results array (`[{ angle, verdict?, findings:[{severity, summary, file?, line?, disposition?}] }]`, the primary shape that feeds `consolidateFanin`); it also accepts the flat per-finding array that `consolidateFanin`/`toFindingsLogShape` produce (`[{ severity, summary, angle?, ... }]`), grouping it by each finding's `.angle`. A non-empty input matching neither shape is rejected rather than silently rendering all-clean. If the structured results are not available (an inline run), fall back to `--findings-summary "<summary>"`. A `fanout_fanin` `consolidate-fanin` round withheld (the absolute structural floor: too many angles to render even hard-truncated) has neither shape available: `--out` was never written (or was removed), and the `--ledger-out` flat file is unbudgeted and MUST NOT be substituted for it — passing that file to `--findings-json` can itself exceed the render budget. Post that round with `--findings-summary "<summary>"` PLUS the mandatory `--findings-ledger <findings-log-path>` (the same durable log Phase 3 wrote via `write-gate-findings-log.mjs --provenance`) — never `--findings-summary` alone: when the gate configures mandatory angles, `upsert-checkpoint-verdict.mjs` proves coverage from that ledger's recorded provenance and refuses the post (naming the missing artifact/angle) when both `--findings-json` and `--findings-ledger` are omitted (see [Phase 3 — Consolidation](../docs/gate-review-sub-loop-contract.md#phase-3--consolidation-fan-in-synthesis-and-disposition-ledger), which owns the full artifact/coverage rule).
...
4. **Fan-out (Phase 2):** resolve this round's dispatch units by calling `resolveFanoutGroups(config, gate, angles, { fullLabel })` (`@dev-loops/core/config`) over the current head's resolved angle set minus the plan's `carried` angles (the Phase 1.2 subtraction — never `mustRerun`); `fullLabel` is whether the PR carries `gate:full` (Phase 1's `--full-label` input). In **grouped mode** (`gates.fanout.mode` unset or `grouped`, the shipped default), configured `gates.fanout.groups` are matched first (unchanged), then the leftover ungrouped angles are auto-chunked by `resolveFanoutGroups` into dispatch units of ≤ `gates.fanout.maxAnglesPerGroup` (N, default 3) instead of singletons — an angle in no configured group joins the leftover pool. `mode: per-angle` bypasses the configured-groups table and emits one singleton unit per angle; it matches `maxAnglesPerGroup: 1` in dispatch unit size ONLY when no configured multi-angle group matches a resolved angle (otherwise per-angle bypasses configured groups while N=1 honors them — matched first, never split — so the two diverge when a configured multi-angle group matches; see the divergence test in `packages/core/test/config.test.mjs`). `gate:full` no longer restores per-angle dispatch (ADR 0048 supersedes 0047): it forces the full angle set upstream (`resolveGateTier` → `gate_full_label`) and dispatches GROUPED — configured groups first, then the leftover pool auto-chunked into units of ≤ N. Turn the resolved plan INTO the per-unit reviewer prompts with the single sanctioned one-shot step `node <resolved-skill-scripts>/github/emit-fanout-dispatch.mjs --repo <owner/name> --pr <n> --gate <gate> --head-sha <sha> [--pending]` (`GATE-EXEC-FANOUT-DISPATCH-EMIT`, issue #2092) — it emits one `{ scope, angles, group, promptPath }` per DISPATCH unit — sharing ONE reviewer for EVERY multi-angle `resolveFanoutGroups` unit, configured `gates.fanout.groups` group or auto-chunk leftover bundle alike (ADR 0048's grouped-dispatch-default), recording the resolved unit's own name as that reviewer's provenance `group`, capped and split at `REVIEWER_UNIT_MAX_ANGLES` exactly as an over-cap configured group already was; only a genuinely single-angle unit dispatches as a singleton (no shared group) — so you never re-derive persona/prompt (the reviewer self-resolves via `resolveReviewerRole`), never spelunk `print-gates.mjs`, and never need to be more conservative than the merge guard by splitting a sanctioned auto-chunk bundle to singletons: the guard (`fanoutReviewerPairingError`, re-deriving `resolveFanoutGroups` independently at both `detect-checkpoint-evidence.mjs` sites) is the fail-closed authority that refuses any shared identity whose covered angles are not all members of that same re-derived unit; the emitter also persists its round plan to the keyed `<gate>-<headSha>.emit-plan.json` sibling of the gate-context bundle — the artifact consumed by Phase 3's `--emit-plan` guard (never a fixed-path stdout capture). When Phase 1.2 carries angles and leaves a non-empty fresh subset, first rebuild the context artifact with `--carried-angles` and `--prev-head <prior_head_sha>` (Phase 1.2's own `--prev-head` value) so `fanout.pendingGroups` is that fresh-only subset and the briefing volatile tail carries AC3's prior-round disposition memory (`resolvePriorDispositions`, issue #2175) for the angles re-running fresh, then pass `--pending` to this emitter command. When Phase 1.2 instead resolves `carried: []` (ambiguity or a fail-closed default forces EVERY angle to re-run fresh, the full-fallback outcome), still rebuild the context artifact with `--prev-head <prior_head_sha>` alone (omit `--carried-angles`, or pass it empty — nothing narrows `pendingGroups`) so the same disposition memory reaches every reviewer on that round too, then dispatch without `--pending` (the full angle set). If every angle is carried, dispatch no emitter; omit `--pending` only when no carry-forward subset exists. Then spawn one scoped `review` agent per emitted unit (plain Agent tool) seeded with its `promptPath` bytes verbatim, recording that unit's `group` on Phase 3's `--provenance`. Wave the EMITTED units at most `maxConcurrent` at a time (the emitter's own output field, `resolveFanoutEffectiveConcurrency` — the configured `gates.fanout.maxConcurrent`, cross-harness default 4, clamped to an effective 2 on the Claude harness per the `docs/decisions/0069-claude-harness-fanout-concurrency-clamp.md` amendment, or 1 when `gates.fanout.sequential` is set), awaiting a free slot (wave completion) before launching the next — instead of fire-all-then-retry (record any degradation in the gate evidence). Do NOT bound this step by `artifact.fanout.wavePlan`: that plan is computed over the UNSPLIT `resolveFanoutGroups` units and no longer matches the emitter's split unit set (an over-cap unit the emitter cap-splits into ceil(N/REVIEWER_UNIT_MAX_ANGLES) sub-units would over-dispatch a single wave slot). When `gates.fanout.sequential` is set (true), the conductor dispatches heavy reviewers **one at a time** — effective concurrency is one dispatch unit per wave regardless of `maxConcurrent` (the wave plan is built with `resolveFanoutEffectiveConcurrency`, emitted as `artifact.fanout.effectiveConcurrency`, issue #1726) — so each heavy reviewer completes and writes its evidence artifact before the next starts, keeping genuine distinct-reviewer fan-out alive under child-safe parallel overload (no collapse to inline; fan-in/ledger/provenance unchanged). Each reviewer follows [review agent scoped angle-review mode](../../agents/review.md): fresh context, read-only, briefed with every angle in its group (each angle's own prompt, all appended after the one shared invariant prefix — `GATE-EXEC-BRIEFING-PREFIX`); when every angle in a dispatch unit declares the SAME non-`full` scope, the conductor ALSO seeds that reviewer with the matching `briefingVariants[scope]` file recorded on the context artifact, layered AFTER the invariant prefix as an additional narrow read — never a replacement (`GATE-EXEC-BRIEFING-PREFIX`'s per-angle scoped variants rule: the reviewer's `--prefix-hash`/`--prefix-file` still records the invariant-prefix bytes regardless of which variant, if any, it was also seeded with) — and any mixed-scope or `full` unit is seeded with the invariant prefix alone, running the mandatory fresh-context guard ONCE per `GATE-EXEC-BRIEFING-PREFIX`'s `--scope` naming rule (`<gate>-group-<name>` for a group, the existing `<gate>-<angle>` for a per-angle dispatch) — not restated here. The reviewer writes its per-angle findings artifact to `tmp/gate-reviews/<repo-slug>/pr-<N>/<gate>-<headSha>/<angle>.json` **once per angle in its group** — a 3-angle group therefore writes 3 artifacts, each with its own verdict and stamped with the reviewed head (`GATE-EXEC-ARTIFACT-HEAD-STAMP`) — never one merged artifact for the group. This keeps fan-in, the ledger, coverage checks, and the head-stamp guard byte-identical to per-angle mode, since all four read per-angle artifacts and never the reviewer that produced them. Grouped-round provenance records the group name on every angle a group's reviewer covered — the ORCHESTRATOR records it on Phase 3's `--provenance` entries below, never the reviewer, whose findings artifact carries no `group` field (`fanoutReviewerPairingError`'s within-group exception in [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md#fan-out-provenance-closing-the-self-produced-artifact-loophole)) — not restated here.
...
5. **Fan-in (Phase 3):** `consolidate-fanin.mjs` now runs `verify-briefing-prefixes.mjs --head-sha <current_head_sha>` itself before consolidation (#1618 — the verifier previously had ZERO callers, so the rule's own cited proof was unenforced); it fails closed on a divergent/hashless sentinel, or when the reviewer sentinel count is short of the dispatch units the conductor spawned. Pass `--expected-dispatch-units <n>` sourced from the Phase 1 context artifact's `fanout.pendingGroups.length` — the dispatched dispatch-UNIT count (groups for grouped dispatch, angle count for per-angle dispatch, where `resolveFanoutGroups` emits one singleton per angle). When Phase 1.2 carries angles forward, whether `pendingGroups` already excludes them depends on whether the Phase 1 context artifact was rebuilt with `write-gate-context.mjs --carried-angles <json>` AFTER Phase 1.2 resolved: rebuilt, `pendingGroups` already excludes every dispatch unit whose angles are ALL carried-or-complete and its length needs no hand-subtraction (a partial carry that empties only SOME of a unit's angles still leaves that unit in `pendingGroups`, and Phase 2 re-chunks the leftover angle set, so the authoritative count is always the count of units the conductor actually dispatched, not `pendingGroups.length` alone — see the sub-loop contract's fan-in section); not rebuilt (the Phase 1 artifact predates Phase 1.2, the common case in this sub-loop's own ordering), `pendingGroups` still includes the carried angles and the caller must subtract the carry-forward-carried dispatch units from `pendingGroups.length` by hand before passing `--expected-dispatch-units`. Either way, if the resulting count is `0` (an all-carried-or-complete round dispatches no reviewer at all), OMIT `--expected-dispatch-units` entirely rather than pass `0` (`consolidate-fanin.mjs` parses it as a positive integer and throws on `0`). NOT `fanout.wavePlan.length` (that is the WAVE count, typically 1, not the dispatch-unit count — passing it would weaken AC3 to a near-noop), and NOT the per-angle artifact count (which would false-fail every grouped round). Then consolidate via ONE call to the sanctioned fan-in CLI — its behavior (blocking severities, emitted shapes, fail-closed cases) is owned by [Gate Review Sub-Loop Contract](../docs/gate-review-sub-loop-contract.md) Phase 3, not restated here: `dev-loops-run cli/index.mjs gate consolidate-fanin --findings-dir <dir> --head-sha <current_head_sha> --gate <gate> --expected-dispatch-units <n> --out <findings-json-path> --ledger-out <ledger-path> --emit-plan <emit-plan-path> --jq '.severityCounts'` (`scripts/loop/consolidate-fanin.mjs`; `--head-sha` activates the `GATE-EXEC-ARTIFACT-HEAD-STAMP` stale-artifact guard — pass it on every round; `--emit-plan <path>` passes the emitter's keyed emit-plan artifact on this sanctioned path for its fail-closed round-key guard, `GATE-EXEC-EMIT-PLAN-KEY`). That ONE invocation both writes `--out`/`--ledger-out` to disk and prints `.severityCounts` on stdout via `--jq` — capture it (e.g. `severity_counts=$(... --jq '.severityCounts')`) for the verdict call's `--findings-severity-counts` below; never re-invoke the CLI a second time just to re-extract a value it already returned. When Phase 1.2 ran and returned a plan, pass BOTH `--carried-angles <json>` (that plan's `carried` angle names) AND `--carry-forward-plan <json>` (Phase 1.2's own result, or at least its `carried` array) — required together, alongside `--gate` (already on this call): the CLI treats `--carried-angles` as proof-carrying, not a bare trust-me list, and fails closed (exit 1) per the sub-loop contract's full refusal list (a configured mandatory or always-include angle, an unmapped name, a name absent from the plan's `carried` list, or a raw artifact self-declaring `carriedFromHead`). Phase 2 dispatched no artifact for a carried angle, so without `--carried-angles` it is invisible to the findings-json/coverage checks and the posted verdict comment reads as a truncated fan-out instead of a full one; omit both flags on a gate's first round (no Phase 1.2 plan). Write the disposition ledger via `write-gate-findings-log.mjs --findings-file <ledger-path> --emit-plan <emit-plan-path> --provenance '<json>' --execution-mode fanout_fanin` (the CLI's `--ledger-out` file, passed straight through — no improvised extraction) before the visible verdict review (`GATE-EXEC-DISPOSITION-LEDGER`, `GATE-EXEC-POST-BEFORE-FIX` own that ordering and the ledger's opt-out-proof durability). Declaring `--execution-mode fanout_fanin` here activates the CLI's write-time fail-closed guard for a gate with mandatory angles: an omitted `--provenance` is now a hard write-time error, not a silent divergence caught only later by the read-time `detect-checkpoint-evidence` check. `--provenance` is a SEPARATE flag on THIS write call — it does not exist on `post-gate-findings.mjs` below (that CLI rejects it as an unknown argument); omit it here and every `carriedFromHead` attribution is dropped, which then silently disables the NEXT round's carry-forward too. Its shape is `{ distinctReviewers: <int>, perAngle: [...] }`: `distinctReviewers` is required, and `perAngle` MUST cover every resolved angle — a fresh entry for each angle Phase 2 dispatched, plus one entry per Phase 1.2 `carried` angle carrying that entry's `carriedFromHead` and its prior reviewer identity fields (`reviewer`/`dispatchId`/`model`) — every identity field it has, not a hand-picked subset, since an entry recorded only under `dispatchId` would otherwise land unattributed. A fresh entry produced by a GROUPED dispatch unit also carries `group` set to that unit's name, repeated identically on every fresh entry the same reviewer covers — you (the conductor), not the reviewer, write this field, since a reviewer's findings artifact has none. On a `gate:full` round also pass `--full-label` to this same `write-gate-findings-log.mjs` call (mirroring Phase 1's `write-gate-context.mjs --full-label`), so its write-time `resolvedGroups` cross-check resolves the same untriered per-angle-singleton table Phase 2 actually dispatched against, instead of the tiered default. The plan's `reason` field is explanatory and is not part of the accepted shape. Passing `--provenance` is doubly load-bearing whenever the gate configures `mandatoryAngles`: `detect-checkpoint-evidence.mjs` enforces mandatory-angle coverage from the ledger's provenance by default for such a gate, independent of `gates.requireFanoutProvenance`, and a ledger with absent or invalid provenance fails closed there. The round's findings ride the verdict review posted in Phase 4, so no separate findings comment is posted; only when a repo has explicitly opted into `gates.postFindingsComments` (default false) also post the same ledger via `node <resolved-skill-scripts>/github/post-gate-findings.mjs --repo <owner/name> --pr <number> --gate <gate> --head-sha <current_head_sha> --findings-file <ledger-path>` — no `--provenance` flag on this call.
6. **Verdict (Phase 4):** check whether the fan-in's `--out` file exists before posting: a withheld round (see the [Gate comment command](#mandatory-gate-comment-command-contract)'s withheld-round note above) deletes `--out` and has no `--findings-json` shape to pass — post that round with `--findings-summary "<summary>"` PLUS `--findings-ledger <findings-log-path>` (the same durable log this same step passes below; never `--findings-summary` alone — see the withheld-round note above for the coverage-proof rule that mandates it). Otherwise post via the [Gate comment command](#mandatory-gate-comment-command-contract) using `--execution-mode fanout_fanin` and `--findings-json <path>` built from the same per-angle artifacts consolidated in Phase 3 — that flag's accepted shapes are owned by the Gate comment command section above; do not restate them here. Pass that same durable log to the verdict call's `--findings-ledger` so the round's findings ride the one posted review (`GATE-EXEC-FINDING-THREADS`). Do NOT run the disposition pass yet — `close-gate-findings.mjs` runs AFTER the Phase 5 (Retry) fixer triage below (#1585: the fixer must see every gate-authored finding, including low findings, before the disposition pass defer-closes them; running it here would silently auto-defer low findings before the fixer ever sees them, reintroducing the #1584 bug).
7. **Retry / fixer triage (Phase 5):** the fixer triages EVERY gate-authored defect finding (high, medium, AND low) and answers every gate-authored question on EVERY gate round — this is UNCONDITIONAL, not gated on "blocking findings" (#1585): on a clean verdict with open low/question threads (the exact #1584 scenario), the fixer STILL runs, receiving each gate-authored thread as a fix/triage/answer target (fix-if-cheap-in-the-same-commit when already touching that code, else defer, for a defect; an answer, promoting or escalating as needed, for a question). The fixer owes a nit thread no triage cycle — it is not handed to the fixer as a fix/triage target on the severity axis, unlike low (the one exception: a judge `act` on a nit reaches the fixer through judge-pass's severity-blind act filter); the disposition pass resolves a still-unresolved nit thread immediately regardless — but NEVER files it to the tracked follow-up issue, and NEVER stamps `disposition=deferred` on it (net-reduction disposition policy, #1846). Defer is permitted from round 1 on for low findings (no forced fix window; the medium window (#1581) is unaffected); whether a fixer-deferred low is ALSO filed and stamped follows the same #1846 gate — only when its own marker carries the finding's explicit `operatorVisible: true` signal (the conservative default, absent/false, is resolved-with-rationale but never filed). Drive blocking findings (high, in-window medium) through the SAME fix → reply-with-resolving-commit → resolve loop used for external Copilot review comments (Step 7 above). Then, at every gate close (clean or not), run `node <resolved-skill-scripts>/github/close-gate-findings.mjs --ledger <findings-log-path> --allowed-refs <governing-issue>` against that same path as the closing sweep — AFTER the fixer triage (#1585): `<governing-issue>` is the PR's governing (closing) issue resolved DETERMINISTICALLY — the `closingIssuesReferences` / `Closes #N` the Phase 1 gate-context bundle already resolved (see Phase 1's "every closing issue reference ... resolved by the CLI itself"), passed as the CSV of all closing issue numbers for an umbrella PR, NEVER a hardcoded literal; omit the flag only when the PR closes no issue. This opens `close-gate-findings.mjs`'s bare-`#N` comment-id guard (shipped in #1992) ONLY for the PR's own governing issue, so a deferred finding whose summary cites that issue dispositions cleanly with no manual flag-threading, while a bare reference to any UNrelated issue stays fail-closed (scoped allowlist, not a blanket bypass). it resolves every still-open DEFERRABLE gate-authored thread (low, nit, and out-of-window medium — the fixer had its chance first on low/medium, owed nothing on nit; it never fix-closes, and leaves high / question / in-window medium unresolved), FILING onto the tracked follow-up issue only the subset that clears the #1846 net-reduction bar (out-of-window medium always; low only when operator-visible; nit never), and REPORTS `unresolvedGateThreadCount` (gate-authored threads still unresolved after the pass, resolved OR filed alike); the gate-close assertion (`fetchDraftGateEvidence` / `ready-for-review` / `pre-pr-ready-gate`) refuses to mark the gate satisfied while that is non-zero. It posts nothing of its own and runs only the thread disposition pass (`GATE-EXEC-THREAD-DISPOSITION`). This is mandatory, not conditional on the verdict. Then re-enter the chain at Phase 1 for the fixed head — Phase 1.2 decides what re-runs. A round that had blocking findings carries nothing (its log is not `clean`), so the fixed head takes the full fan-out; carrying only becomes possible on the head bumps that follow a clean round. Context-builder and fan-in always re-run. Repeat until the consolidated verdict is `clean` for the current head SHA AND 0 unresolved gate-authored threads remain (`GATE-EXEC-REGATE-MANDATORY`, `GATE-EXEC-THREAD-DISPOSITION`).
