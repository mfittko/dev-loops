...
**Pre-flight PR gate (mandatory):** Before working an existing PR, the dev-loop must run `npx dev-loops@1.0.2 loop handoff --repo <owner/name> --pr <number>` and abort if `action: "stop"`. When `terminal: true`, proceed inline. When `terminal: false`, resolve the blocking condition first.
...
Info/handoff requests can be served directly via `npx dev-loops@1.0.2 loop info` (read-only; no full dev-loop run required):
- `npx dev-loops@1.0.2 loop info --issue <n>` — human-readable issue state summary (strategy, route, linked PR, next action)
- `npx dev-loops@1.0.2 loop info --pr <n>` — human-readable PR state summary (branch, CI, threads, rounds, action)
- `npx dev-loops@1.0.2 loop info --issue <n> --json` — machine-readable JSON output
...
```sh
content_digest=$(node <resolved-skill-scripts>/loop/spec-context.mjs --repo <owner/name> --issue <linked_issue_number> \
...
```sh
node <resolved-skill-scripts>/loop/spec-context.mjs changed-paths --base <prior_approved_head_sha> --head <current_head_sha> \
...
```sh
npx dev-loops@1.0.2 gate consolidate-fanin --findings-dir <dir> --head-sha <current_head_sha> --gate <gate> \
...
```sh
npx dev-loops@1.0.2 gate judge-pass --repo <owner/name> --pr <N> --gate <gate> --head-sha <current_head_sha> \
