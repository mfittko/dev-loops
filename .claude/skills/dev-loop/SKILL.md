...
Resolve authoritative state via the startup resolver (`npx dev-loops@1.0.0-rc.4 loop startup --issue <n>` for issues, `npx dev-loops@1.0.0-rc.4 loop startup --pr <n>` for PRs), then immediately build the handoff envelope via `npx dev-loops@1.0.0-rc.4 loop build-envelope --input <resolver-output.json>`. The envelope determines `requiredReads`, `nextAction`, `stopRules`, and `acceptance` — load only those files, execute only that bounded task. It is the first handoff artifact consumed before loading any route pack. See [Workflow Handoff Contract](../docs/workflow-handoff-contract.md) for the derivation contract.

**Retrospective checkpoint gate:** the resolver reads `.pi/dev-loop-retrospective-checkpoint.json` and, when the repo config `workflow.requireRetrospective` (set via `.devloops` at repo root) is `true`, also queries GitHub live for the latest qualifying completion and compares it against the checkpoint's recorded `identity` (repo + PR number + merge commit) — a `complete`/`skipped` record whose identity does not match the latest one it finds is treated as `missing`, so a stale record can never satisfy a newer qualifying cycle. When the resulting state is `missing`, the resolver returns `needs_reconcile`. Complete or explicitly skip the retrospective before starting, carrying the cycle identity via `checkpoint-contract.mjs --repo`/`--pr`/`--merge-commit`.

**Pre-flight PR gate (mandatory):** Before working an existing PR, the dev-loop must run `npx dev-loops@1.0.0-rc.4 loop handoff --repo <owner/name> --pr <number>` and abort if `action: "stop"`. When `terminal: true`, proceed inline. When `terminal: false`, resolve the blocking condition first.
...
Info/handoff requests can be served directly via `npx dev-loops@1.0.0-rc.4 loop info` (read-only; no full dev-loop run required):
- `npx dev-loops@1.0.0-rc.4 loop info --issue <n>` — human-readable issue state summary (strategy, route, linked PR, next action)
- `npx dev-loops@1.0.0-rc.4 loop info --pr <n>` — human-readable PR state summary (branch, CI, threads, rounds, action)
- `npx dev-loops@1.0.0-rc.4 loop info --issue <n> --json` — machine-readable JSON output
