...
**Pre-flight PR gate (mandatory):** Before working an existing PR, the dev-loop must run `npx dev-loops@1.0.0-rc.6 loop handoff --repo <owner/name> --pr <number>` and abort if `action: "stop"`. When `terminal: true`, proceed inline. When `terminal: false`, resolve the blocking condition first.
...
Info/handoff requests can be served directly via `npx dev-loops@1.0.0-rc.6 loop info` (read-only; no full dev-loop run required):
- `npx dev-loops@1.0.0-rc.6 loop info --issue <n>` — human-readable issue state summary (strategy, route, linked PR, next action)
- `npx dev-loops@1.0.0-rc.6 loop info --pr <n>` — human-readable PR state summary (branch, CI, threads, rounds, action)
- `npx dev-loops@1.0.0-rc.6 loop info --issue <n> --json` — machine-readable JSON output
