...
Info/handoff requests can be served directly via `npx dev-loops@0.7.0 loop info` (read-only; no full dev-loop run required):
- `npx dev-loops@0.7.0 loop info --issue <n>` — human-readable issue state summary (strategy, route, linked PR, next action)
- `npx dev-loops@0.7.0 loop info --pr <n>` — human-readable PR state summary (branch, CI, threads, rounds, action)
- `npx dev-loops@0.7.0 loop info --issue <n> --json` — machine-readable JSON output
