...
The four lenses' findings are merged deterministically by the pure converge seam (`scripts/loop/ui-review-lenses.mjs`): the route hands your flat `findings[]` to `convergeUiReviewRouteFindings(findings, { acceptanceCriteria, checkedCriteria })` — passing the `acceptanceCriteria` list and your `checkedCriteria` passes straight through — which groups them by `lens` and calls `convergeUiReviewLenses`. The seam fails closed unless a non-empty `acceptanceCriteria` list is provided, so the route MUST forward it. Findings sharing a normalized `(stateName, region, category)` triple are deduped, the worse severity wins, and the outcome enum is derived (a must-fix finding ⇒ not satisfied; a `blocking` design conflict ⇒ blocked; an acceptance criterion left uncovered ⇒ not satisfied). Emit raw per-lens findings plus the `checkedCriteria` passes; do not dedupe or pick the outcome yourself.
