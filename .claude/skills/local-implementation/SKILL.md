...
- <!-- rule: LOCAL-PR-CREATE-CANONICAL --> for tracker-backed sessions, you MUST open PRs through the canonical `dev-loops-run cli/index.mjs pr create` path — always draft and always assigned — self-assigned by default (`--assignee @me` when none is given; honors an explicit `--assignee <login>` / `-a <login>`) — and MUST NOT open them via raw `gh pr create`. Author the PR body from the canonical conformant skeleton at [PR Body Skeleton](../dev-loop/templates/pr-body.md): a tracker-backed body MUST carry Objective, In scope, Acceptance criteria + Definition of done checklists, an explicit Non-goals section, and a `Closes #N` (or `Fixes #N`) reference for the linked issue so GitHub auto-closes it on merge. That same `validate-pr-body-spec` contract now runs at BOTH ends — `dev-loops-run cli/index.mjs pr create` fails closed at create time on a non-conformant body, mirroring the ready-for-review boundary — so author it conformant up front instead of discovering it non-conformant when marking ready. When `.devloops` sets `workflow.requireDraftFirst` to true, use `dev-loops-run cli/index.mjs pr create --assignee @me ...`; the always-draft creation mechanism is owned by [`OPS-DRAFT-FIRST-PR`](../docs/copilot-loop-operations.md) — `create-pr.mjs` is unconditionally draft-only regardless of the toggle, which separately governs whether the draft-gate boundary is enforced before ready.
...
- <!-- rule: LOCAL-DELEGATE-SELF-COMMIT --> `developer`/`quality`/`docs`/`fixer` dispatches COMMIT THEIR OWN WORK before exit per [LOCAL-COMMIT-BEFORE-EXIT](#implementation-loop-for-the-phase) (step 11); for a session that pushes and opens a PR (the scope step 11b defines: tracker-backed or issue-less `--lightweight`) the first push is deferred to the pre-PR review step (11b) so the branch reaches origin once, already cleaned. There is no "edit here, commit there" split: an editing sub-delegate is never told not to commit, and an orchestrator that wants a single consolidated commit performs the edits itself rather than delegating the edit and keeping the commit. The removed `DEVLOOPS_ORCHESTRATOR_OWNS_COMMIT` exemption must not be used; `subagent-stop-uncommitted-guard` remains enforced for every editing role.
...
11. <!-- rule: LOCAL-COMMIT-BEFORE-EXIT --> **Exit validation gate — no uncommitted changes.** Run `git status --porcelain` before exiting. Revert unintended/speculative deltas; validate intended changes with the narrowest justified check, then commit. For a session that pushes and opens a PR (the scope step 11b defines: tracker-backed or issue-less `--lightweight`) the first push is deferred to the pre-PR review step (11b), so pre-PR fixes land before the branch ever reaches origin; phase-doc-backed sessions do not push (step 13 merges locally). Recheck that status is empty before declaring phase completion. Dirty exit is a workflow defect, never clean completion. The `SubagentStop` hook (`.claude/hooks/subagent-stop-uncommitted-guard.mjs`) refuses dirty exits under `tmp/worktrees/`, naming the rule and paths. Read-only `judge`/`review` roles (`READONLY_SUBAGENT_ROLES`) are exempt: their foreign tracked edits produce an advisory naming the orchestrator as responsible. Enforcement remains for the orchestrator and every editing role (`developer`, `fixer`, `docs`, `quality`); post-merge cleanup can otherwise destroy uncommitted work.
11b. <!-- rule: LOCAL-PRE-PR-REVIEW-BEFORE-PUSH --> **Pre-PR review pass (before the first push).** For any session that pushes and opens a PR — tracker-backed OR issue-less `--lightweight` (PR-body-as-spec); only a phase-doc-backed local-merge session is exempt — after the step-11 commit and before the branch's first push and PR creation, run the pre-PR review phase per [Pre-push review contract](../docs/pre-pr-review-contract.md): hand a mandatory developer-authored review brief to ONE fresh-context general-purpose reviewer, paired with the adversarial-enumeration checklist. Resolve the reviewer model via `resolveRoleModel(config, { role: "pre-push-reviewer", harness })` and pass it only when non-null; on Claude dispatch the built-in `general-purpose` subagent with the brief as its prompt, on Pi dispatch one fresh subagent with the brief as its task (a general-purpose reviewer; the PR-bound `review` gate procedure does not apply). The reviewer returns findings only and MUST NOT edit the tree; the implementer applies the fixes. Apply findings in-tree, rerun the narrowest justified validation on the fixes (re-check after the final round), then commit them (follow-up commit or amend) so `git status --porcelain` is empty, and only THEN push once. The pass is bounded to one reviewer per round and at most two rounds, is ephemeral (no PR/comment/thread/Copilot), and is NOT lifecycle-gate evidence. The full fan-out `draft_gate`/`pre_approval_gate` still runs afterward as the authority.
