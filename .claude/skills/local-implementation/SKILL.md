...
11b. <!-- rule: LOCAL-PRE-PR-REVIEW-BEFORE-PUSH --> **Pre-PR review pass (before the first push).** For tracker-backed sessions, after the step-11 commit and before the branch's first push and PR creation, run the pre-PR review phase per [Pre-PR review contract](../docs/pre-pr-review-contract.md): hand a mandatory developer-authored review brief to ONE fresh-context general-purpose reviewer, paired with the adversarial-enumeration checklist. Resolve the reviewer model via `resolveRoleModel(config, { role: "pre-PR-reviewer", harness })` and pass it only when non-null; on Claude dispatch the built-in `general-purpose` subagent with the brief as its prompt, on Pi dispatch one fresh subagent with the brief as its task (a general-purpose reviewer; the PR-bound `review` gate procedure does not apply). The reviewer returns findings only and MUST NOT edit the tree; the implementer applies the fixes. Apply findings in-tree, commit them (follow-up commit or amend) so `git status --porcelain` is empty, and only THEN push once. The pass is bounded to one reviewer and at most two rounds, is ephemeral (no PR/comment/thread/Copilot), and is NOT lifecycle-gate evidence. The full fan-out `draft_gate`/`pre_approval_gate` still runs afterward as the authority.
