...
1. Fetch comments into their own JSON file, extracting past the wrapper's envelope: for **tracker-first**, `dev-loops-run scripts/github/view-issue.mjs --repo <owner/repo> --issue <n> --json comments --jq '.issue.comments' > <comments-path>` (never raw `gh`); for **PR body**, the same with `scripts/github/view-pr.mjs ... --jq '.pr.comments' > <comments-path>`. The `--jq` extraction is required: the wrapper's own output is `{ "ok": true, "issue"|"pr": { "comments": [...] } }`, and the detector's `--comments-file` accepts only a bare array or `{ "comments": [...] }` — feeding it the unextracted envelope fails closed.
2. Run `dev-loops-run scripts/loop/detect-refinement-grill-state.mjs --body-file <body-path> --surface <issue|pr> --comments-file <comments-path>`.
3. Take the zero-iteration exit ONLY when the detector's output has `state == grill_clean`. Any other state (including `detect_gaps` with `reason: provenance_missing`) means the target is NOT a zero-iteration exit — run the full grill (Steps 2–4) anyway; a zero-gap pass still records its own provenance by posting the results comment (Step 4), stating that no gaps were found.
