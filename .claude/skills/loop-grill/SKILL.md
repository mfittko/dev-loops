...
1. Fetch comments into their own JSON file, extracting past the wrapper's envelope: for **tracker-first**, `dev-loops-run scripts/github/view-issue.mjs --repo <owner/repo> --issue <n> --json comments --jq '.issue.comments' > <comments-path>` (never raw `gh`); for **PR body**, `dev-loops-run scripts/github/view-pr.mjs --repo <owner/repo> --pr <n> --json comments --jq '.pr.comments' > <comments-path>`. The `--jq` extraction is required: the wrapper's own output is `{ "ok": true, "issue"|"pr": { "comments": [...] } }`, and the detector's `--comments-file` accepts only a bare array or `{ "comments": [...] }` — feeding it the unextracted envelope fails closed.
