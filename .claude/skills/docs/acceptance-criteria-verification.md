...
> **Non-tracker spec-source fork:** when the linked issue body is not the spec-of-record — the resolver output / handoff envelope carries `specSource: pr_body` (`canonicalSpecSource: pr_body`, lightweight) or `specSource: plan_file` (plan-file promotion) — the fork covers three arms: lightweight, which splits on whether a backing issue exists, and plan-file promotion, a distinct local-planning origin.
>
