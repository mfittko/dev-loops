...
The HARD guarantee these three layers give: no two contributors ever both PROCEED past their own startup gate on the same artifact — every contested or foreign ownership read fails closed, so double work never happens. Convergence to a solely-owned (or cleanly unassigned) item holds for the common races: a sequential claim race, or any race where at least one racer's post-claim re-read actually observes the contention (the tiebreak above then self-heals it). The rare residual — accepted rather than denied, because GitHub assignment has no compare-and-swap: a near-simultaneous double-claim where BOTH racers' post-claim re-reads land before the other's write propagates leaves EACH seeing only its own claim, so neither observes the contention and no tiebreak ever fires. That leaves the item genuinely co-assigned — safe, not stuck-and-broken, since both contributors' own startup gates still see the contested state and fail closed (no double work) — but it does need a human to unassign one login, exactly the documented non-goal: no lease/expiry or automatic stale-claim reclamation; an abandoned or contested assignment is released manually.
