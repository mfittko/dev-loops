...
Together these layers guarantee the TERMINAL state: every interleaving converges to a solely-owned (or cleanly unassigned) item with no stuck co-assignment and no manual intervention, and any contributor whose ownership read observes a contested or foreign state fails closed. Because GitHub assignment has no compare-and-swap, one residual window remains and is accepted rather than denied: if one racer's entire claim → re-read → startup-read sequence completes before the other racer's first claim call lands, both can transiently start before the winner's removal propagates — narrow enough that the next ownership read on either side (any watch cycle, gate, or restart re-derives ownership from scratch) fails the loser closed. Double work is made improbable and short-lived by construction, not physically impossible.
