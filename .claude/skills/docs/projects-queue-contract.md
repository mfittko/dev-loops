...
- <!-- rule: QUEUE-ENQUEUE-REFINEMENT-GATE --> **An issue MUST carry the full refinement matrix before it enters `Next Up`.** When an enqueue resolves the target to the pickup column (`queue add --next-up`, or `--column` naming it), it gates on the same check the draft gate uses: the #1877 full AC/DoD/Non-goals matrix — an Acceptance criteria checklist, a Definition of done checklist, and an explicit `## Non-goals` section (see `detectIssueRefinementArtifact` and [Artifact Authority Contract](artifact-authority-contract.md) rule `ARTIFACT-TRACKER-ISSUE-REFINEMENT-FLOOR`). A linked refinement doc (`tmp/refinement/*.md`) remains a complete artifact on its own. An AC-only issue blocks with `missing_dod_checklist`, a DoD-only issue with `missing_ac_checklist`, and a matrix-missing Non-goals section with `missing_explicit_non_goals` — each with its own guidance naming the missing arm. An un-refined issue never lands in the pickup column in the first place. The gate is issue-only: a PR targeting the pickup column is not gated here (its spec-of-record is validated at the draft gate). Interactive enqueue **MUST** fail closed with `MISSING_REFINEMENT_ARTIFACT`, naming the missing sections. Headless (`--auto`) enqueue **MUST NOT** fail the run; it diverts the issue to the non-pickup park column (`nonSuccessBoardColumn`, which **MUST** differ from the pickup column) and records the diversion reason instead. Synthesizing the missing artifact (running the refiner / `loop-grill --auto`) is an **orchestration-layer** responsibility, not the board-mutation script's: the enqueue script enforces the gate and parks; the agent/command layer grills a parked issue and re-enqueues once refined. This shifts the check left; the draft gate remains the unconditional backstop for whatever slips through some other enqueue path.
