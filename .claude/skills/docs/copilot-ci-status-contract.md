...
`gate-evidence` (`.github/workflows/gate-evidence.yml`) is a server-side check whose conclusion is DERIVED from the loop's own progress (a clean current-head `pre_approval_gate` verdict) — not an independent build/test signal. The dev-loop must never let it block the very step (posting `pre_approval_gate`) that would turn it green, and must not treat it as "unconfirmed" CI either: every reason it can be non-green — evidence not yet established for the current head (reported as a neutral `pending`, not a red failure), unresolved threads, a stale runner, or a genuine current-head `pre_approval_gate` violation (reported as `failure`) — is independently tracked elsewhere in the loop snapshot. The exclusion applies whether the status reads `pending` or `failure`. `LOOP_DERIVED_CI_CHECK_NAME` is the single exported constant naming this check; `partitionEntriesByCheckName` is the shared primitive `deriveLoopCiStatusFromRollup` composes from, and the current-head refresh in `scripts/loop/detect-copilot-loop-state.mjs` reuses the same constant and exclusion rule on BOTH refresh probes — the check-runs probe and the commit-status probe (`gate-evidence` is posted as an explicit commit `StatusContext`, matched by `.context`, not a check-run) — so the fallback (rollup) and refresh (check-runs + commit-status) derivation paths never disagree.
