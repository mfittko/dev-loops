...
7. ✅ Closing-reference state matches artifact backing: tracker-backed work and issue-less lightweight (PR-body-as-spec, no backing issue) are owned by `ARTIFACT-LIGHTWEIGHT-BODY-INVARIANTS` in [Artifact Authority Contract](artifact-authority-contract.md) — tracker-backed work: PR body contains `Closes #N` or `Fixes #N`; issue-less lightweight: the closing reference is absent by design and `node scripts/loop/validate-pr-body-spec.mjs --repo <owner/name> --pr <number> --no-issue` passes clean. Plan-file promotion (P4) stands on its own authority (not `ARTIFACT-LIGHTWEIGHT-BODY-INVARIANTS`, which does not cover it): the PR body carries the committed plan-doc path as the spec-of-record and, being issue-less by design (`buildPromotionPrBody` neutralizes closing keywords), the closing reference MUST NOT be present
