...
7. ✅ Closing-reference state matches artifact backing: tracker-backed work — PR body contains `Closes #N` or `Fixes #N`; issue-less lightweight (PR-body-as-spec, no backing issue) — `node scripts/loop/validate-pr-body-spec.mjs --repo <owner/name> --pr <number> --no-issue` passes clean, and a closing reference MUST NOT be present; plan-file promotion (P4) — the PR body carries the committed plan-doc path as the spec-of-record
