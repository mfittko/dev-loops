...
Dispatch one independent, fresh-context `review` agent per emitted unit via the plain Agent tool, seeded verbatim with the neutral bundle and its angle prompts. Never inherit the conductor's or a sibling reviewer's context. Follow the [review agent's scoped angle-review mode](../../agents/review.md). On Pi these emitted units are released by `emit-wave-dispatch.mjs` as ONE `runs.all` wave per wave — run it at the round's key and issue ONE `subagent` call per emitted wave (`GATE-EXEC-FANOUT-WAVE-DISPATCH`), never one call per unit; the agent-authored harnesses (Claude Code / Codex) dispatch one `review` agent per emitted unit.
