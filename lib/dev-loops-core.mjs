...
// Direct dev-loop entrypoints: thin named wrappers over the public dev-loop contract.
...
// `continue` targets an issue OR a PR (the resolver picks the canonical artifact) and
...
// Surface-neutral: the lifecycle family is available wherever the calling
// runtime supplies a `uiLifecycle` (Pi extension and the `dev-loops` CLI).
function parseInspectCommand(tokens) {
  const [, rawAction, ...rawArgs] = tokens;
  const action = rawAction?.toLowerCase();
  if (!INSPECT_ACTIONS.has(action)) {
    return invalidCommand('`inspect` only supports: open, resume, status, stop, restart.', 'inspect', tokens);
  }
...
// verbs intact. It is a thin wrapper over the shipped `--spike` intake:
...
export function parseDevLoopsCommand(input, { surface = 'extension' } = {}) {
  ...
  if (action === 'inspect') {
    return parseInspectCommand(tokens);
  }
...
// ── Inspect viewer lifecycle presentation (CLI + Pi extension) ─────
// Both surfaces render the same lines and classify severity the same way; only
// the sink differs (stdout + exit code vs Pi widget + notification).
// 🟢 healthy, 🟠 recoverable (a record or port needs cleanup), 🔴 the action failed.
export function inspectStateEmoji(result) {
  if (inspectResultSeverity(result) === 'info') {
    return '🟢';
  }
  return result.state === 'stale_record' || result.state === 'conflict_unmanaged_listener' ? '🟠' : '🔴';
}

export function renderInspectLines(action, result) {
  const lines = [
    `inspect ${action}`,
    `State: ${inspectStateEmoji({ action, ...result })} ${result.state}`,
  ];
  if (result.repo) {
...
// `stopped` is a success state for `stop`/`status` (nothing was running there)
// and a failure for every other action, so severity stays fail-closed by
// default. The detail-text branches cover lifecycle results without `record`.
export function inspectResultSeverity(result) {
  if (result.state === 'running') {
    return 'info';
  }
  if (result.state !== 'stopped' || (result.action !== 'stop' && result.action !== 'status')) {
    return 'error';
  }
  ...
  if (result.action === 'stop' && /stopped the managed inspect-run viewer/i.test(detail)) {
    return 'info';
  }
  if (result.action === 'status' && /no managed inspect-run viewer is recorded/i.test(detail)) {
    return 'info';
  }
  return 'error';
}
...
export async function executeDevLoopsCommand({ input, surface = 'extension', runtime, stdout }) {
  ...
  if (parsed.kind === 'inspect_action') {
    if (typeof runtime?.uiLifecycle?.[parsed.action] !== 'function') {
      return {
        kind: 'unsupported',
        message: 'Inspect lifecycle commands need a viewer lifecycle runtime; this surface did not provide one.',
