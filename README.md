...
The hooks provide the `gh pr ready` draft-gate guard, an opt-in read-only boundary for the main agent (`DEVLOOPS_MAIN_AGENT_READONLY=1`), and an opt-in coordinator→worker delegation boundary — a dispatched dev-loop coordinator must delegate tracked-repo-file mutations and code-verification/build commands (`bun run verify`/`bun test`/`vitest`/`npm test`/`npm run test`, and the analogous `build` script across `bun`/`npm`/`yarn`/`pnpm`) to a worker subagent rather than run them inline (`DEVLOOPS_COORDINATOR_READONLY=1`). Skill references to a consumer repo's own `PLAN.md` / `AGENTS.md` resolve against that repo, by design.

**Consumer entry contract.** The plugin ships prompts, skills, agents, and hooks, plus a hand-authored `.claude/package.json` + `.claude/package-lock.json` that pin the published `dev-loops` npm package to the release version. Claude Code's native plugin dependency auto-install runs `npm ci --ignore-scripts` at cache time, landing the full toolchain under `.claude/node_modules/dev-loops/` (only the manifest and lockfile are committed — never `node_modules`). Every generated skill/command/agent invocation runs through the `dev-loops-run` launcher on the plugin `bin/` PATH, which resolves the dev-loops root in order: a live source checkout found by walking up from the current directory always wins unconditionally (so local edits are dogfooded with no release), else a live source checkout found by walking up from the launcher's own real binary location, else the auto-installed `.claude/node_modules/dev-loops`, else a loud hard-stop that names the unresolved wrapper — never a raw `node scripts/*.mjs` against the checkout, and never a silent raw-`gh` fallback. The single-source CLI subcommand reference is tracked in [issue #1376](https://github.com/mfittko/dev-loops/issues/1376).
...
- **Refinement** — fan-out count and mode for parallel review variants; `refinement.maxCopilotRounds` caps Copilot re-review rounds (default `5`). Set `maxCopilotRounds: 0` to disable the Copilot gate entirely — local-harness-only review (`draft_gate → pre_approval_gate`), useful when the repo has no Copilot reviewer configured. By default one converged Copilot review stands for later heads, and `pre_approval_gate` reviews each head; set `refinement.requireCopilotConvergenceAtLatestHead: true` to open a new Copilot cycle for every significant post-convergence change.
