...
The hooks provide the `gh pr ready` draft-gate guard, an opt-in read-only boundary for the main agent (`DEVLOOPS_MAIN_AGENT_READONLY=1`), and an opt-in coordinator→worker delegation boundary — the dev-loop coordinator itself may not mutate tracked repo files or run code-verification/build commands inline (`bun run verify`/`bun test`/`vitest`/`npm test`/`npm run test`, and the analogous `build` script across `bun`/`npm`/`yarn`/`pnpm`); only a worker subagent may (`DEVLOOPS_COORDINATOR_READONLY=1`). Skill references to a consumer repo's own `PLAN.md` / `AGENTS.md` resolve against that repo, by design.
