...
- [ ] Per-merge retrospective checkpoint write (`checkpoint-contract.mjs --state complete --repo <owner/name> --pr <n> --merge-commit <full oid> --retro-context fresh --record-source <tool-call-record-path>` — identity flags and fresh-context provenance are mandatory for `complete`)
