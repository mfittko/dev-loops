...
The trigger is a session property: the session pushes and opens a PR. `PRE-PR-BEFORE-FIRST-PUSH` in `skills/docs/pre-pr-review-contract.md` is route-neutral. The GitHub-first routes reach the existing step at `OPS-DRAFT-FIRST-PR` in `skills/docs/copilot-loop-operations.md`, before the first push and before `create-pr.mjs`. Step 11b stays the local route's instance of the same step. The review itself is unchanged: one fresh-context reviewer, at most two rounds, ephemeral, and never gate evidence.
