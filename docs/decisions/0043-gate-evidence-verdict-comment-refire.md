...
The workflow also triggers on `issue_comment` (`created`, `edited`), gated at the job level to PR-attached comments that start with the gate-comment marker (`### Gate review:`) and come from a trusted author (`OWNER`/`MEMBER`/`COLLABORATOR`), so neither ordinary discussion nor untrusted marker-shaped comments start a run; because `issue_comment` payloads carry no `pull_request` object, a Resolve-PR-facts step resolves number/draft/head SHA once for all downstream steps. `created` covers each verdict posted on a new head (the upsert creates a fresh comment per head); `edited` covers a same-head verdict update (the upsert edits the existing comment when the verdict content changes) and the manual recovery (editing the comment by id via `scripts/github/edit-comment.mjs` re-fires the check); only an identical same-head rerun no-ops. Evaluation always checks out the DEFAULT BRANCH (trusted code, `persist-credentials: false`); the resolved PR head SHA is used only as the explicit status target. Runs coalesce per PR through a job-level concurrency group (job-level deliberately: a workflow-level group would be joined by marker-skipped runs, letting ordinary comments cancel live evaluations), with superseded runs cancelled and barred from posting via `!cancelled()`. Recovery for a lost run when the verdict comment already exists is editing that comment (the `edited` event); `gh run rerun` is documented as a non-recovery because it replays the stale original payload.
