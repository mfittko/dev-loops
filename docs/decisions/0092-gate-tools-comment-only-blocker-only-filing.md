...
- No defer-close of an open act thread. `close-gate-findings.mjs` never selects a thread whose finding the judge disposed `act`, whatever its severity and round. The current round's ledger decides first; on no match there, the prior local ledgers decide, then the thread's rendered judge suffix. An ambiguous result at either ledger tier also skips the thread: current-ledger entries with the thread's fingerprint whose dispositions disagree (mixed or partly missing), or prior ledgers that disagree. The fixer replies to every gate thread it fixed or declined on reproduction grounds with the fixing commit or a decline reason, and resolves it before `close-gate-findings.mjs` runs.
