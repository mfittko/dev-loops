...
- No defer-close of an open act thread. `close-gate-findings.mjs` never selects a thread whose finding the judge disposed `act`, whatever its severity and round. The current round's ledger decides first; on no match there, the prior local ledgers decide, then the thread's rendered judge suffix. An ambiguous prior-ledger result also skips the thread. The fixer replies to every gate thread it fixed with the fixing commit and resolves it before `close-gate-findings.mjs` runs.
