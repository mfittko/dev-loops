...
- One comment per round. A round's deferred findings, including the operator-visible folded findings, go as one batched comment on the target. `followUpIssueNumber` in the ledger and `issue=<n>` in the thread marker record the target's number. A finding whose fingerprint the target already lists is not appended again.
- No defer-close of an open act thread. `close-gate-findings.mjs` never selects a thread whose finding the current round's ledger disposes `act`, whatever its severity and round. The fixer replies to every gate thread it fixed with the fixing commit and resolves it before `close-gate-findings.mjs` runs.
