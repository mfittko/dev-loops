...
- The check fails closed. An existing ledger copy that does not parse into an object with a `findings` array refuses merge and is named, even when a copy in another checkout parses. A `fanout_fanin` `pre_approval_gate` ledger with at least one finding that carries no `judgeDisposition` refuses merge until the ledger is written with `--judge-verdict`. A ledger with zero findings needs no judge. Unjudged findings refuse merge unless the posted `pre_approval_gate` marker's `executionMode` is `inline_single_agent`; the ledger's own `executionMode` never exempts it.
- The durable findings log's `judgeDisposition` is the act-list authority for the posted verdict and for merge. The ledger carries the judge pass's spec-authority enforcement: when `write-gate-findings-log.mjs` gets both `--judge-verdict` and `--spec-authority`, it validates the sibling `spec-authority-verdict.json` against that identity and writes each `finding_conflicts` finding as `reject`, through the helper the judge pass uses (`rejectFindingConflicts` in `packages/core/src/loop/spec-authority.mjs`). An invalid or mismatched sibling verdict writes no ledger. An absent one changes nothing. The judge's disposition rules are unchanged.
- A judge `act` overrides the medium fix window and surface-time deferral for the verdict. An act item blocks `clean` until a fix commit is re-gated. An act item closed without a commit (an answered question, or a finding that does not reproduce) needs a judge rerun at the current head, the ledger rewritten with `--judge-verdict`, and the verdict re-posted. The merge refusal names this recovery path.
