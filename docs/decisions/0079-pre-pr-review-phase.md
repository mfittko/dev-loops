...
The first push is deferred to step 11b for any session that pushes and opens a PR (tracker-backed or issue-less `--lightweight` PR-body-as-spec): the per-slice commit push (implementation-loop step 3), the sub-delegate self-commit push (`LOCAL-DELEGATE-SELF-COMMIT` and its canonical home in `main-agent-contract.md`), and the exit-validation push (`LOCAL-COMMIT-BEFORE-EXIT`) all commit locally and defer the first push to 11b, so the branch reaches origin once, already cleaned and validated. Only a phase-doc-backed local-merge session has no first push and no pre-PR step.
