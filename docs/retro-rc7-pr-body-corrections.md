...
1. **PR 1854** — title "records-floor closed (completes #1468 cache-align)" and DoD
   ...
   been fixed (merged to main in PR #1881).
...
- **PR 1854** — the relevant title fragment restated as "records-floor closed on the
  canonical path only (completes #1468 cache-align; zero-records residual tracked by
  #1868)" (the full new title keeps its leading "feat(gate): deterministic reviewer-prompt
  composer — inline-prefix-first by construction, " prefix); body carries a
...
- **PR 1847** — DoD line restated as deterministic-only: the interactive manual step was
  ...
  (<https://github.com/mfittko/dev-loops/pull/1461#pullrequestreview-5062828929>). Provenance record comment:
