...
3. **Accurate command surface.** Command names match the code: Claude Code plugin commands are `/dev-loops:loop-*` (generated 1:1 from `commands/*.command.md`); Pi exposes them as `/dev-loops <sub>`. Distinguishes dev-loop entrypoints from top-level utilities (`status`, `doctor`, `gates`). No invented or renamed commands.
...
5. **Accurate install / config / requirements.** Version examples use a `<version>` placeholder (never a hardcoded release). The shipped-defaults path is correct (`packages/core/src/config/extension-defaults.yaml`), with `.devloops` as the consumer override. Universal requirements (Node ≥24, authenticated `gh`) are separated from Pi-harness-only ones (`pi-subagents`, the `@earendil-works/*` peer deps).
