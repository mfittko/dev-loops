...
- code-correctness angles whose surface excludes `docs` (`scope`, `correctness`, `coverage`, `determinism`, …) → surface includes `code`/`test`/`config`; a pure doc delta does not touch it, so it carries forward.
