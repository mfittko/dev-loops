...
deslop); the Slidev source tracks the slide headlines but is not regenerated from, so
edit the `.html` for anything that ships and mirror only headline changes back to the
`.md`.
