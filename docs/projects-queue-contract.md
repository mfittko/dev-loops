...
| Reprioritize or block the item | The item's column **MUST** be updated to match: reprioritized items move between `Backlog` and `Next Up`, with promotion into `Next Up` subject to the same [QUEUE-ENQUEUE-REFINEMENT-GATE](#queue-pickup-ordering) as any other pickup-column entry; a blocked item **MUST** be moved out of the active `Next Up`/`In Progress` columns back to `Backlog`. |
