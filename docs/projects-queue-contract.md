...
- <!-- rule: QUEUE-ENQUEUE-REFINEMENT-GATE --> **An item MUST carry a refinement artifact before it enters `Next Up`.** `queue add`/`--next-up` gates enqueue on the same check the draft gate uses (an Acceptance criteria section, a DoD section, or a linked refinement doc — see `detectIssueRefinementArtifact`), so an un-refined issue never lands in the pickup column in the first place. Interactive enqueue **MUST** fail closed with `MISSING_REFINEMENT_ARTIFACT`, naming the missing sections. Headless (`--auto`) enqueue **MUST NOT** fail the run; it diverts the item to the non-pickup park column (`nonSuccessBoardColumn`) and records the diversion reason instead. This shifts the check left; the draft gate remains the unconditional backstop for whatever slips through some other enqueue path.
