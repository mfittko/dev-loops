...
| Reprioritize or block the item | The item's column **MUST** be updated to match: reprioritized items move between `Backlog` and `Next Up`, but the conductor **MUST NOT** promote an unrefined item into `Next Up` — [QUEUE-ENQUEUE-REFINEMENT-GATE](#queue-pickup-ordering) bars unrefined issues from the pickup column on the enqueue path, and the conductor upholds that same bar on the reprioritization/move path, which the enqueue-time check does not cover; a blocked item **MUST** be moved out of the active `Next Up`/`In Progress` columns back to `Backlog`. |
