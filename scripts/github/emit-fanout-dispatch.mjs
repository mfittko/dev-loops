...
export function buildAngleNamingSuffix(unit) {
  ...
  const body = single
    ? `Resolve this angle's persona and focus prompt by running the sanctioned CLI \`dev-loops-run cli/index.mjs gate resolve-role --angle <name>\` (fill <name> with the angle named above), then review adversarially per your scoped angle-review mode. If the CLI reports \`fallback: true\` or a null/empty \`prompt\` (\`status\` \`fallback\`/\`prompt-missing\`), review the angle by name with no angle-specific focus instruction — never grep the shipped defaults and never treat the fallback as a blocked reviewer. Write one findings artifact for this angle at its per-angle path.`
    : `For EACH angle above, resolve its persona and focus prompt by running the sanctioned CLI \`dev-loops-run cli/index.mjs gate resolve-role --angle <name>\` (once per angle, filling <name> with that angle), then review adversarially per your scoped angle-review mode. If the CLI reports \`fallback: true\` or a null/empty \`prompt\` (\`status\` \`fallback\`/\`prompt-missing\`) for an angle, review that angle by name with no angle-specific focus instruction — never grep the shipped defaults and never treat the fallback as a blocked reviewer. Write one findings artifact PER ANGLE at its per-angle path — one artifact per angle, never one merged artifact for the unit.`;
