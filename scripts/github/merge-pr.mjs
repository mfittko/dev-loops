...
Preconditions (each refuses with a machine-readable reason naming the failing one):
  ...
  earlier head, with zero unresolved threads and no Copilot review outstanding
  on the current head; the delta is not checked),
  round_cap_clean_fallback (round cap reached, unless the last review was clean
  and a significant change landed since), or, with
  refinement.requireCopilotConvergenceAtLatestHead: true, docs_only_suppression
  (the loop's carried convergence holds across a docs-only or integrate-only
  delta, and no Copilot review is outstanding on the current head). gate_evidence reuses
...
Output (stdout, JSON): { ok, merged, mergeCommit, approvedBy, mergeClass, approvalVia, method, repo, pr, headSha, copilotConvergenceState, copilotDisposition, copilotCarriedConvergence, copilotBodyDisposition, postMerge }
  ...
  copilotCarriedConvergence: for converged_once and docs_only_suppression, the carried review
...
// Copilot review converge, in order: the Copilot gate is disabled (cap 0);
// converged-once (the default mode: the loop's shared predicate finds the
// latest Copilot review converged on an earlier head, whatever the delta);
// the round cap is exhausted (round-cap clean fallback); in strict mode, the
// same predicate carries the prior review across a docs-only or
// integrate-only delta (docs-only suppression); last, an internal-only PR
// (the loop's reviewMode internal_only, which skips the Copilot cycle) maps
// to copilot_gate_disabled. Returns `{ kind, headSha }` pinned to the current
// head, plus `carriedConvergence` (the source review and its commit) for
// converged_once and docs_only_suppression, or null (copilot_convergence then
// refuses).
async function resolveCopilotAbsentReviewDisposition({ repo, pr, currentHeadSha, rawReviews, config, draftGate, lightweight }, runtime) {
  ...
  const requireCopilotConvergenceAtLatestHead = resolveRequireCopilotConvergenceAtLatestHead(config);
  ...
  const carriedDisposition = async (kind) => {
    ...
    return { ...pinned(kind), carriedConvergence: { source, sourceReviewId, sourceHeadSha, bodyDisposition } };
  };
  if (!requireCopilotConvergenceAtLatestHead) {
    const convergedOnce = await carriedDisposition(COPILOT_ABSENT_REVIEW_DISPOSITION.CONVERGED_ONCE);
    if (convergedOnce) return convergedOnce;
  }
  ...
  // reopened the cycle: the loop honors a docs-only or integrate-only carry in
  // both cases (the raw probe skips the base-relative reduction), so merge
  // consults the same predicate.
  if (requireCopilotConvergenceAtLatestHead) {
    const docsOnly = await carriedDisposition(COPILOT_ABSENT_REVIEW_DISPOSITION.DOCS_ONLY_SUPPRESSION);
    if (docsOnly) return docsOnly;
  }
...
export async function mergePr(options, runtime = {}) {
  ...
  // copilot-body-disposition record the loop and gate entry honor. The resolver
  // reads the same review list as the raw convergence state (no draft-gate
  // round reset), so it names the same current-head review the state judged.
