...
- `suppressed_post_convergence` is the default converged-once suppression (`refinement.requireCopilotConvergenceAtLatestHead: false`, ADR 0090): the latest Copilot review converged on an earlier head, zero review threads are unresolved, and no request is outstanding, so no fresh round is opened below the cap, at the cap, or under `--force-rerequest-review`, whatever the delta. The round-cap return carries `completedRounds` and `maxRounds`
- `suppressed_post_convergence_docs_only` is the strict-mode (`refinement.requireCopilotConvergenceAtLatestHead: true`) or operator-marker suppressed (non-request) outcome returned at the round cap AND, since #2316, below it, whenever the delta since the last Copilot-reviewed head is a provable pure doc/prose bump OR an integrate-only base-move (base-relative reduction empties the delta), so no fresh blocking round is forced. The round-cap return, like `round_cap_reached`, carries `completedRounds` and `maxRounds`; the below-cap return omits both fields
...
- `draftGate.currentHeadClean` (ADR 0088) also folds in the login-narrowed `unresolvedGateThreadCount`: it is `true` only when the marker verdict is clean AND no gate-authored review thread (e.g. an unanswered or not-yet-reject-closed `question`) is still unresolved; `preApprovalGate.currentHeadClean` never receives this signal and keeps the marker-only definition. `draftGate.markerCleanThreadsUnresolved` is `true` when the marker is clean but a real thread still dangles; `draftGate.markerCleanThreadStateUnreadable` is `true` when the thread count itself could not be read (fail-closed -1)
