...
async function makeRiskPathDocsDiffRepo() {
  ...
  await writeFile(path.join(repoRoot, "docs", "decisions", "0001-old.md"), "# Old\n", "utf8");
  git(repoRoot, ["add", "-A"]);
  git(repoRoot, ["commit", "-q", "-m", "base"]);
  const baseSha = git(repoRoot, ["rev-parse", "HEAD"]).trim();
  await writeFile(path.join(repoRoot, "docs", "decisions", "0001-old.md"), "# New\nMore detail.\n", "utf8");
  git(repoRoot, ["add", "-A"]);
  git(repoRoot, ["commit", "-q", "-m", "risky docs"]);
  const headSha = git(repoRoot, ["rev-parse", "HEAD"]).trim();
  return { repoRoot, baseSha, headSha };
}
...
test("buildGateContext falls back to pointer mode when the diff exceeds the inline cap, and discloses it", async () => {
  ...
  try {
    ...
    assert.ok(onDisk.includes(result.artifact.scope.diffPath), "evidence discloses the diffPath pointer");
...
test("#1601 resolveFanoutDispatch: gate:full dispatches grouped (no per-angle restoration)", () => {
  ...
  const plan = resolveFanoutDispatch(config, "draft", ["a", "b", "c"], { fullLabel: true, env: {} });
...
test("#1601 buildGateContextArtifact records the fanout dispatch plan when supplied", () => {
  // env pinned non-Claude (#1971) — see the wave-shape rationale above.
  const plan = resolveFanoutDispatch({ version: 1, gates: { fanout: { maxConcurrent: 1 } } }, "draft", ["a", "b"], { env: {} });
  const artifact = buildGateContextArtifact({
    repo: "a/b", pr: 5, gate: "draft_gate", headSha: "abc1234",
    angles: ["a", "b"],
    fanoutDispatch: plan,
...
test("#1726 gates.fanout.sequential serializes the wave plan and records sequential/effectiveConcurrency in the fanout artifact", () => {
  // env pinned non-Claude (#1971) — see the wave-shape rationale above.
  const plan = resolveFanoutDispatch({ version: 1, gates: { fanout: { mode: "per-angle", sequential: true, maxConcurrent: 8, maxAnglesPerGroup: 1 } } }, "draft", ["a", "b", "c"], { env: {} });
...
test("#1726 sequential:false keeps the maxConcurrent wave plan (no serialization)", () => {
  // env pinned non-Claude (#1971) — see the wave-shape rationale above.
  const plan = resolveFanoutDispatch({ version: 1, gates: { fanout: { mode: "per-angle", sequential: false, maxConcurrent: 2 } } }, "draft", ["a", "b", "c"], { env: {} });
...
test("writeGateContext persists the evidence file and binds it, the diff, and validation by sha256 in artifact.requiredReads and the prefix", async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(reads).sort(), ["context", "diff", "evidence", "validation"]);
...
test("writeGateContext: an over-cap diff makes the full diff a required read", async () => {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "gate-context-required-diff-"));
  try {
    const diffOutput = `diff --git a/x.mjs b/x.mjs\n${"+line\n".repeat(Math.ceil(BRIEFING_PREFIX_INLINE_DIFF_CAP_BYTES / 6) + 10)}`;
    const diffPath = buildGateDiffPath({ repo: "o/r", pr: 3, gate: "draft_gate", headSha: "abc1234" });
    const result = await writeGateContext({
      ...parseWriteGateContextCliArgs(["--repo", "o/r", "--pr", "3", "--gate", "draft_gate", "--head-sha", "abc1234", "--angles", '["scope"]']),
      diffOutput, diffPath, diffToWrite: { path: diffPath, text: diffOutput },
    }, { repoRoot });
    assert.equal(result.prefixMode, "pointer");
