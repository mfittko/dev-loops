...
import { buildFindingMarker, findOwnSubmittedReview, fingerprintFinding } from "../../scripts/github/_gate-finding-surface.mjs";
    ...
    const env = await writeGhStub(tempDir, [
      ...
      },
      {
        ...
        assertStdinIncludes: [
          ...
          "**Size-budget waiver:** granted by jane-doe",
        ],
        stdout: '{"id":101,"html_url":"https://github.com/owner/repo/pull/17#pullrequestreview-101"}\n',
      },
    ]);
...
test("upsert-checkpoint-verdict --size-budget-json fails closed on a malformed .outcome", async () => {
  await withTempDir(async (tempDir) => {
    ...
    // Same gh call sequence as the successful create path, up through listing
    // existing comments — the malformed --size-budget-json is read (and
    // refused) BEFORE the review POST, so no POST entry is provided; a call
    // that reached it would fail the mock instead of matching this test's
    // expected error.
    const env = await writeGhStub(tempDir, [
      {
        assertArgs: ["pr", "view", "17", "--repo", "owner/repo", "--json", "number,state,isDraft,headRefOid,mergeable,mergeStateStatus,body,title,closingIssuesReferences,reviews,statusCheckRollup,files"],
  ...
  await withTempDir(async (tempDir) => {
    ...
    const env = await writeGhStub(tempDir, [
      ...buildGateCoordinationEntries({
        ...
        issueComments: [buildGateComment({ headSha })],
...
test("upsert-checkpoint-verdict refuses a pre_approval_gate post on a ready PR with no clean draft_gate evidence (no gate exemptions, #579)", async () => {
  await withTempDir(async (tempDir) => {
    const env = await writeGhStub(tempDir, [
      {
        matchByClaims: true,
      ...
      // No draft_gate comment exists for this PR (Copilot is converged and CI is
      // green, but the draft-gate boundary was never reconciled).
      {
        assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/issues/17/comments?per_page=100"],
        stdout: `${JSON.stringify([[]])}\n`,
      },
    ...
    const payload = JSON.parse(result.stderr);
    assert.equal(payload.ok, false);
    assert.match(payload.error, /Cannot enter pre_approval_gate/);
    assert.match(payload.error, /no gate exemptions, #579/i);
    assert.match(payload.error, /run reconcile_draft_gate before proceeding/i);

    // Critical: no review verdict was posted, and the PR was never converted to draft.
    const postedReview = result.calls.some((call) => call.args.includes("repos/owner/repo/pulls/17/reviews"));
    assert.equal(postedReview, false, "must not POST a review verdict");
    const convertedToDraft = result.calls.some((call) => call.args.some((arg) => /convertPullRequestToDraft/.test(arg)));
    assert.equal(convertedToDraft, false, "must not convert the PR to draft");
  }, { prefix: "dev-loops-upsert-pre-approval-refused-no-draft-evidence-" });
});
...
// result on the created/noop paths; absent it is a pure no-op.
test("upsert-checkpoint-verdict: --spec-authority stamps the returned created/noop result; absent is a no-op", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "dev-loops-upsert-gate-review-spec-authority-"));
  const headSha = "abc1234000000000000000000000000000000000";
  const identity = {
  ...
  const identityPath = path.join(tempDir, "spec-authority.json");
  ...
  try {
    ...
    const created = await upsertCheckpointVerdict(
      { ...inputs, specAuthority: identityPath },
      { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", runChild: runChild1, repoRoot: fanoutDisabledRepoRoot },
    );
    assert.equal(created.action, "created");
    assert.deepEqual(created.specAuthority, { ...identity, checkedCriteria: ["ac:0", "ac:1"] });
    const postedBody = JSON.parse(calls1.find((c) => c.args.includes("repos/owner/repo/pulls/17/reviews")).stdinText).body;

    // Absent --spec-authority: no specAuthority field at all (pure no-op).
    const { runChild: runChild2 } = makeGhMock([
      ...buildGateCoordinationEntries({ isDraft: true, statusCheckRollup: [{ __typename: "CheckRun", status: "COMPLETED", conclusion: "SUCCESS" }] }),
      {
        assertArgs: ["api", "-X", "POST", "repos/owner/repo/pulls/17/reviews", "--input", "-"],
        stdout: '{"id":104,"html_url":"https://github.com/owner/repo/pull/17#pullrequestreview-104"}\n',
      },
    ], { repeatLastOnOverflow: true });
    const createdNoStamp = await upsertCheckpointVerdict(
      inputs,
      { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", runChild: runChild2, repoRoot: fanoutDisabledRepoRoot },
    );
    assert.equal("specAuthority" in createdNoStamp, false);

    // Same-head noop: the stamp still carries through.
    const { runChild: runChild3 } = makeGhMock([
      ...buildGateCoordinationEntries({
        isDraft: true,
        statusCheckRollup: [{ __typename: "CheckRun", status: "COMPLETED", conclusion: "SUCCESS" }],
        issueComments: [{ id: 103, body: postedBody, html_url: "https://github.com/owner/repo/pull/17#issuecomment-103", updated_at: "2026-08-03T00:00:00Z" }],
    ...
    const noop = await upsertCheckpointVerdict(
      { ...inputs, specAuthority: identityPath },
      { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", runChild: runChild3, repoRoot: fanoutDisabledRepoRoot },
    );
    assert.equal(noop.action, "noop");
    assert.deepEqual(noop.specAuthority, { ...identity, checkedCriteria: ["ac:0", "ac:1"] });
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("renderGateReviewCommentBody omits the size-budget lines entirely when sizeOutcome is not supplied (back-compat)", () => {
  const body = renderGateReviewCommentBody({
    gate: "pre_approval_gate",
    headSha: "abc1234000000000000000000000000000000000",
    verdict: "clean",
  ...
  assert.ok(!body.includes("Size-budget"));
  const parsed = parseGateReviewCommentBody(body);
  // Read side: absent evidence, not a spurious "pass".
...
test("renderGateReviewCommentBody round-trips a pass outcome with no T1 slice and no waiver", () => {
  const body = renderGateReviewCommentBody({
    gate: "pre_approval_gate",
    headSha: "abc1234000000000000000000000000000000000",
    verdict: "clean",
  ...
  assert.match(body, /\*\*Size-budget outcome:\*\* pass/);
  assert.match(body, /\*\*Size-budget T1 slice:\*\* not touched/);
  assert.match(body, /\*\*Size-budget waiver:\*\* none/);
  const parsed = parseGateReviewCommentBody(body);
  assert.equal(parsed.sizeOutcome, "pass");
  assert.equal(parsed.sizeTouchesT1, false);
  assert.equal(parsed.sizeWaiverGranted, false);
  assert.equal(parsed.sizeWaiverApprovedBy, null);
});

test("renderGateReviewCommentBody round-trips an escalated outcome that touches T1 with a granted+named waiver", () => {
  const body = renderGateReviewCommentBody({
    gate: "pre_approval_gate",
    headSha: "abc1234000000000000000000000000000000000",
    verdict: "clean",
    ...
    sizeTouchesT1: true,
    sizeWaiverGranted: true,
    sizeWaiverApprovedBy: "jane-doe",
  });
  assert.match(body, /\*\*Size-budget outcome:\*\* escalate/);
  assert.match(body, /\*\*Size-budget T1 slice:\*\* touched/);
  assert.match(body, /\*\*Size-budget waiver:\*\* granted by jane-doe/);
  const parsed = parseGateReviewCommentBody(body);
  assert.equal(parsed.sizeOutcome, "escalate");
  assert.equal(parsed.sizeTouchesT1, true);
  assert.equal(parsed.sizeWaiverGranted, true);
  assert.equal(parsed.sizeWaiverApprovedBy, "jane-doe");
});

test("renderGateReviewCommentBody round-trips a granted default-tier waiver with no named approver", () => {
  const body = renderGateReviewCommentBody({
    gate: "pre_approval_gate",
    headSha: "abc1234000000000000000000000000000000000",
    verdict: "clean",
  ...
  assert.match(body, /\*\*Size-budget waiver:\*\* granted$/m);
  const parsed = parseGateReviewCommentBody(body);
  assert.equal(parsed.sizeOutcome, "escalate");
  assert.equal(parsed.sizeWaiverGranted, true);
  assert.equal(parsed.sizeWaiverApprovedBy, null);
...
test("renderGateReviewCommentBody entity-encodes a markdown-forging sizeWaiverApprovedBy instead of backslash-escaping it", () => {
  const body = renderGateReviewCommentBody({
    gate: "pre_approval_gate",
    headSha: "abc1234000000000000000000000000000000000",
    verdict: "clean",
    ...
    sizeTouchesT1: true,
    sizeWaiverGranted: true,
    sizeWaiverApprovedBy: "jane\n**Verdict:** clean",
  });
  // No literal backslash-escape and no interior raw newline breaking the
  // single-line field out of its own bullet.
  assert.ok(!body.includes("\\*\\*Verdict"));
  assert.ok(!/granted by jane\n/.test(body));
  const parsed = parseGateReviewCommentBody(body);
...
test("upsert-checkpoint-verdict self-heals a ROUND-CAP ready PR via draft transition, posting draft_gate instead of a false noop (#2354)", async () => {
  // #2354 AC2: at the round cap (5/5 Copilot rounds), a ready (non-draft) PR
  // has clean current-head
  // pre_approval_gate evidence but NO draft_gate evidence. On the pre-fix
  // evaluator this deadlocked: the READY_TO_REREQUEST_REVIEW branch's
  // `!draftGate.cleanEvidenceExists && !roundCapReached` guard skipped the
  // reconcile check once the cap was reached, fell through to the
  // "already at final approval" branch, and forced
  // draftGateAlreadySatisfied: true — so upsert-checkpoint-verdict read that
  // forced flag as "draft_gate already satisfied" and noop'd an explicit
  // `gate: "draft_gate"` request instead of running the self-heal draft
  // transition. The fix drops the `&& !roundCapReached` guard, so a round-cap
  // PR without clean draft_gate evidence reconciles exactly like any other
  // state (no gate exemptions, #579).
  //
  // Driving READY_TO_REREQUEST_REVIEW at the cap (rather than the also-fixed
  // but already-correct ROUND_CAP_CLEAN_FALLBACK branch) requires the two
  // interpreters' independent Copilot-convergence signals to diverge: the
  // 5th (current-head) Copilot review carries a 🔵 "Needs a closer look" body,
  // which copilot-loop-state.mjs's `cleanThreads` treats as unresolved body
  // feedback (so its own round-cap block leaves `state` undecided), while
  // pr-gate-coordination.mjs's independent `copilotBodyConvergence` treats a
  // thread-clean 🔵 as converged. Combined with a genuinely outstanding
  // (stranded) Copilot review request lodged AFTER the 5th round settles,
  // copilot-loop-state.mjs's round-cap block defers to its normal routing,
  // which lands on UNRESOLVED_FEEDBACK_PRESENT; pr-gate-coordination.mjs's
  // gate-entry reconciliation then promotes that to READY_TO_REREQUEST_REVIEW
  // while roundCapReached stays true — the exact deadlock shape.
  await withTempDir(async (tempDir) => {
    const headSha = "abc1234000000000000000000000000000000000";
    const cleanPreApprovalComment = {
      id: 501,
    ...
    // Four completed rounds on prior heads, then the 5th (the round cap) on
    // the current head with a thread-clean 🔵 body — convergence-ok, but a
    // body finding by copilot-loop-state.mjs's own reading.
    ...
    const requestedReviewersStdout = '{"users":[{"login":"copilot-pull-request-reviewer"}],"teams":[]}\n';
    // A `review_requested` timeline event AFTER the 5th round settles — the
    // stranded in-flight request that keeps copilot-loop-state.mjs's round-cap
    // block from resolving ROUND_CAP_CLEAN_FALLBACK on its own.
    const timelineStdout = JSON.stringify({
    ...
    // Claims-mode (order-independent), mirroring the #891 self-heal test: pass
    // 1 sees isDraft:false (yields reconcile); after convertPullRequestToDraft
    // the recursive post re-enters and pass 2 sees isDraft:true (posts
    // normally). Each pass independently resolves the Copilot-requested +
    ...
    const ghEntries = [
      ...
      { assertArgs: ["pr", "view", "17", "--repo", "owner/repo", "--json", "headRefOid"], stdout: '{"headRefOid":"abc1234000000000000000000000000000000000"}\n' },
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/issues/17/comments?per_page=100"], stdout: JSON.stringify([[cleanPreApprovalComment]]) + "\n" },
      { assertArgs: ["api", "repos/owner/repo/issues/17/timeline"], stdout: timelineStdout },
      { assertArgs: ["pr", "view", "17", "--repo", "owner/repo", "--json", "files"], stdout: "src/index.ts\n" },
      // --- resolve PR node id + convert to draft ---
      { assertArgs: ["api", "graphql", "name=repo", "number=17"], stdout: '{"data":{"repository":{"pullRequest":{"id":"PR_node","isDraft":false}}}}\n' },
      { assertArgs: ["api", "graphql", "pullRequestId=PR_node"], stdout: '{"data":{"convertPullRequestToDraft":{"pullRequest":{"id":"PR_node","isDraft":true}}}}\n' },
    ...
    // The round cap must never force a false noop: the poster must have run
    // the self-heal draft transition and CREATED the draft_gate verdict.
    assert.equal(result.ok, true);
    assert.equal(result.action, "created");
    assert.equal(result.gate, "draft_gate");
    assert.equal(result.executionMode, "fanout_fanin");
...
test("upsert-checkpoint-verdict self-heals draft_gate at the round cap without a false noop when pre_approval evidence is missing (#2354)", async () => {
  // #2354 AC2/AC3: at the round cap, a ready (non-draft) PR with NO clean
  // current-head pre_approval_gate evidence and no draft_gate evidence lands on
  // the ROUND_CAP_CLEAN_FALLBACK non-clean-head branch, which the core
  // evaluator's own guard now rewrites to DRAFT_GATE_NEEDED / reconcile_draft_gate
  // (no clean draft_gate evidence at any of the five guarded boundaries, the
  // round cap never substitutes for it). Because coordination now explicitly
  // allows RECONCILE_DRAFT_GATE, an explicit `gate: "draft_gate"` request takes
  // the self-heal draft-transition path — never a false noop, and
  // draftGateAlreadySatisfied stays false throughout.
  await withTempDir(async (tempDir) => {
    const headSha = "abc1234000000000000000000000000000000000";
    ...
    // Claims-mode (order-independent) matching across the TWO coordination
    // passes: pass 1 sees isDraft:false (yields reconcile); after
    // convertPullRequestToDraft the recursive post re-enters and pass 2 sees
    ...
    // draft_gate configures mandatory angles (pr-description, holistic); a
    // fanout_fanin verdict requires coverage proof rather than a bare
    // findingsSummary.
    const findingsPath = path.join(tempDir, "findings.json");
    await writeFile(findingsPath, JSON.stringify([
      { angle: "pr-description", verdict: "clean", findings: [] },
    ...
    // The round cap must never force a false noop: the verdict was posted via
    // the self-heal transition (never the noop action, which is the only
    // path that would report draftGateAlreadySatisfied: true) — evidence was
    // genuinely missing, not forced true by the round cap.
    assert.equal(result.ok, true);
    assert.equal(result.action, "created");
    assert.equal(result.gate, "draft_gate");
    assert.equal(result.draftTransition, true);
    assert.notEqual(result.draftGateAlreadySatisfied, true);

    const ghLog = JSON.stringify(calls.map(({ args }) => args));
    assert.ok(/convertPullRequestToDraft/.test(ghLog), "expected a convertPullRequestToDraft mutation");
    assert.ok(/\["pr","ready","17"/.test(ghLog.replace(/\s/g, "")), "expected a `pr ready` call to restore the ready state");
  }, { prefix: "dev-loops-upsert-round-cap-self-heal-" });
});
...
function reviewGateFindingSurfaceEntries({ issueComments = [], reviews = [], threads = [], files = [], pendingReviews = [], submittedReviews = [] } = {}) {
  return [
    ...
    ...(files === null ? [] : [{ assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/files?per_page=100"], stdout: JSON.stringify(files) + "\n" }]),
    // findOwnSubmittedReview scan: same-head own SUBMITTED review dedup, run
    // AFTER resolveFindingSurface (it needs the resolved locatable split).
    // Default [] = no prior submitted review, so the create path is taken; the
    // `api user` author-identity read only fires when a header/head candidate
    // matches, so it is omitted for the default empty scan.
    { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/reviews?per_page=100"], stdout: JSON.stringify(submittedReviews) + "\n" },
    ...(submittedReviews.length > 0 ? [{ assertArgs: ["api", "user"], stdout: '{"login":"gate-bot"}\n' }] : []),
  ];
}
...
test("#1912: a STALE own pending review on a DIFFERENT head is DELETEd before falling through to create (GitHub's one-pending-per-PR limit is PR-scoped, so leaving it would 422 the create)", async () => {
  await withTempDir(async (tempDir) => {
    ...
    const entries = [
      ...
      // findOwnSubmittedReview scan (post-resolveFindingSurface): no own
      // submitted review, so the create path proceeds.
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/reviews?per_page=100"], stdout: "[]\n" },
      {
        assertArgs: ["api", "-X", "POST", "repos/owner/repo/pulls/17/reviews", "--input", "-"],
...
test("#1912: a fanout_fanin round posted with --findings-json and NO --findings-ledger emits a one-line warning naming --findings-ledger as the inline-comment source", async () => {
  await withTempDir(async (tempDir) => {
    ...
    const entries = [
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/reviews?per_page=100"], stdout: "[]\n" },
      // findOwnSubmittedReview scan: no own submitted review (no ledger means
      // resolveFindingSurface issues no reads, so this is the next call).
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/reviews?per_page=100"], stdout: "[]\n" },
      {
        assertArgs: ["api", "-X", "POST", "repos/owner/repo/pulls/17/reviews", "--input", "-"],
...
test("#1621: a findings_present draft_gate verdict DERIVES the next action, ignoring an advancing caller value", async () => {
  ...
  try {
    ...
    const result = await upsertCheckpointVerdict({
      ...
      nextAction: "merge",
      executionMode: "inline_single_agent", inlineReason: "inline test",
...
test("#1621: a blocked draft_gate verdict DERIVES 'stay draft and fix' (not the caller's advancing action)", async () => {
  ...
  try {
    ...
    const result = await upsertCheckpointVerdict({
      ...
      nextAction: "mark ready for review",
      executionMode: "inline_single_agent", inlineReason: "inline blocked test",
    }, { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", repoRoot, runChild });
    assert.equal(result.action, "created");
    const postCall = calls.find((c) => c.args.some((x) => x.includes("pulls/17/reviews")) && c.args.includes("POST"));
    assert.ok(postCall, "a review was posted");
...
test("#1621: an inline round whose --findings-json carries a blocking-severity finding applies gate:full (structuredFindings branch)", async () => {
  ...
  try {
    ...
    const result = await upsertCheckpointVerdict({
      ...
      findingsSummary: "ignored in structured mode",
      nextAction: "stay draft and fix",
      executionMode: "inline_single_agent", inlineReason: "inline structured test",
    }, { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", repoRoot, runChild });
    assert.equal(result.action, "created");
    assert.equal(result.gateFullLabelApplied, true);
...
test("#1621: a blocking-severity finding with a resolved disposition does NOT escalate gate:full (structuredFindings branch)", async () => {
  ...
  try {
    ...
    const result = await upsertCheckpointVerdict({
      ...
      findingsSummary: "ignored in structured mode",
      nextAction: "mark ready for review",
      executionMode: "inline_single_agent", inlineReason: "inline structured resolved test",
    }, { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", repoRoot, runChild });
    assert.equal(result.action, "created");
    assert.equal(result.gateFullLabelApplied, undefined);
    assert.equal(calls.some((c) => c.args[0] === "label" && c.args.includes("gate:full")), false);
  ...
  try {
    ...
    const existingComment = {
      ...
      body: renderGateReviewCommentBody({
        ...
        executionMode: "inline_single_agent", inlineReason: "inline update test",
      }),
      html_url: "https://github.com/owner/repo/pull/17#issuecomment-91",
  ...
  try {
    ...
    await assert.rejects(
      () => upsertCheckpointVerdict({
        ...
        findingsSeverityCounts: { high: 0, medium: 0, low: 0 },
        nextAction: "await final human approval",
        executionMode: "inline_single_agent", inlineReason: "inline umbrella test",
      }, { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", repoRoot, runChild }),
      (err) => {
        assert.match(err.message, /Cannot set verdict "clean" for pre_approval_gate/);
        assert.match(err.message, /ACCEPT-CRITERIA-VERIFY-AND-REFLECT/);
...
// ---- #2389: the fan-in ledger's overallVerdict is the REVIEW verdict; for
// pre_approval_gate the checkpoint verdict composes it with the deterministic
// AC/DoD blockers (review blocked -> blocked; any blocker -> blocked; else the
// review verdict). ----

const COMPOSITION_UNCHECKED_DOD_PR_BODY = [
  "## Acceptance criteria", "",
  "- [x] first AC is done", "",
  "## Definition of done", "",
  "- [ ] post-merge smoke run recorded", "",
  "## Non-goals", "",
  "- none", "",
].join("\n");
const COMPOSITION_UNCHECKED_AC_PR_BODY = [
  "## Acceptance criteria", "",
  "- [x] first AC is done", "- [ ] follow-up tracked in #123", "",
  "## Definition of done", "",
  "- [x] tests pass", "",
  "## Non-goals", "",
  "- none", "",
...
async function withCompositionRound({ overallVerdict, prBody, gate = "pre_approval_gate", requireFanoutEvidence = false, extraComments = [] }, fn) {
  ...
  try {
    ...
    const post = (overrides = {}) => upsertCheckpointVerdict({
      ...
      nextAction: gate === "draft_gate" ? "mark ready for review" : "await final human approval",
      findingsLedger: ledgerPath,
...
test("#2389: composeCheckpointVerdict implements the pre-approval composition table", () => {
  const blocker = [{ kind: "unchecked PR-body Definition of done", item: "x" }];
  assert.equal(composeCheckpointVerdict({ reviewVerdict: "clean", blockers: [] }), "clean");
  assert.equal(composeCheckpointVerdict({ reviewVerdict: "findings_present", blockers: [] }), "findings_present");
  assert.equal(composeCheckpointVerdict({ reviewVerdict: "blocked", blockers: [] }), "blocked");
  assert.equal(composeCheckpointVerdict({ reviewVerdict: "blocked", blockers: blocker }), "blocked");
  assert.equal(composeCheckpointVerdict({ reviewVerdict: "clean", blockers: blocker }), "blocked");
  assert.equal(composeCheckpointVerdict({ reviewVerdict: "findings_present", blockers: blocker }), "blocked");
});
...
test("#2389: collectPreApprovalGateBlockers reads the three clean-guard sources and nothing else", () => {
  ...
  assert.deepEqual(collectPreApprovalGateBlockers(gate, {
    ...
    { kind: "unticked spec-of-record Acceptance criteria", item: "spec ac" },
...
test("#2389 regression: a clean ledger plus an unchecked PR-body DoD item posts blocked at the current head with no --verdict, before any fix", async () => {
  await withCompositionRound({ overallVerdict: "clean", prBody: COMPOSITION_UNCHECKED_DOD_PR_BODY }, async ({ post, postedBody, calls }) => {
    const result = await post();
    assert.equal(result.action, "created");
    assert.equal(result.gate, "pre_approval_gate");
    assert.equal(result.headSha, GATE_FULL_HEAD);
...
test("#2389: a findings_present ledger plus an unchecked AC item posts blocked and records the findings_present review verdict", async () => {
  await withCompositionRound({ overallVerdict: "findings_present", prBody: COMPOSITION_UNCHECKED_AC_PR_BODY }, async ({ post, postedBody }) => {
    const result = await post();
    assert.equal(result.action, "created");
    const body = postedBody();
    assert.match(body, /\*\*Verdict:\*\* blocked/);
...
test("#2389: an explicit blocked verdict over a completed ledger is accepted when a blocker is proven", async () => {
  await withCompositionRound({ overallVerdict: "clean", prBody: COMPOSITION_UNCHECKED_DOD_PR_BODY }, async ({ post, postedBody }) => {
    const result = await post({ verdict: "blocked" });
    assert.equal(result.action, "created");
    assert.match(postedBody(), /\*\*Review verdict:\*\* clean/);
  });
});

for (const prBody of [FULLY_TICKED_PR_BODY, COMPOSITION_UNCHECKED_DOD_PR_BODY]) {
  test(`#2389: a blocked ledger always yields blocked (${prBody === FULLY_TICKED_PR_BODY ? "complete" : "incomplete"} AC/DoD)`, async () => {
    await withCompositionRound({ overallVerdict: "blocked", prBody }, async ({ post, postedBody }) => {
      const result = await post();
      assert.equal(result.action, "created");
      const body = postedBody();
      assert.match(body, /\*\*Verdict:\*\* blocked/);
      // Both layers agree, so no separate review-verdict line is rendered.
      assert.doesNotMatch(body, /\*\*Review verdict:\*\*/);
    });
  });
}

test("#2389: a clean ledger with complete AC/DoD posts clean exactly as before", async () => {
  await withCompositionRound({ overallVerdict: "clean", prBody: FULLY_TICKED_PR_BODY }, async ({ post, postedBody }) => {
    const result = await post({ findingsSeverityCounts: { high: 0, medium: 0, low: 0 } });
    assert.equal(result.action, "created");
    const body = postedBody();
    assert.match(body, /\*\*Verdict:\*\* clean/);
    assert.doesNotMatch(body, /\*\*Review verdict:\*\*|\*\*Gate blockers:\*\*/);
  });
});

test("#2389: a findings_present ledger with complete AC/DoD posts findings_present exactly as before", async () => {
  await withCompositionRound({ overallVerdict: "findings_present", prBody: FULLY_TICKED_PR_BODY }, async ({ post, postedBody }) => {
    const result = await post();
    assert.equal(result.action, "created");
    const body = postedBody();
    assert.match(body, /\*\*Verdict:\*\* findings_present/);
    assert.doesNotMatch(body, /\*\*Review verdict:\*\*|\*\*Gate blockers:\*\*/);
  });
});

test("#2389 negative: findings_present over a clean ledger is still refused (no blocker)", async () => {
  await withCompositionRound({ overallVerdict: "clean", prBody: FULLY_TICKED_PR_BODY }, async ({ post }) => {
    await assert.rejects(() => post({ verdict: "findings_present" }), /contradicts the consolidated ledger's overallVerdict "clean"/);
  });
});

test("#2389 negative: findings_present over a clean ledger is still refused when a blocker is proven, naming the composed verdict", async () => {
  await withCompositionRound({ overallVerdict: "clean", prBody: COMPOSITION_UNCHECKED_DOD_PR_BODY }, async ({ post }) => {
...
test("#2389 negative: clean over a findings_present ledger is still refused", async () => {
  await withCompositionRound({ overallVerdict: "findings_present", prBody: FULLY_TICKED_PR_BODY }, async ({ post }) => {
    await assert.rejects(
      () => post({ verdict: "clean", findingsSeverityCounts: { high: 0, medium: 0, low: 0 } }),
      /contradicts the consolidated ledger's overallVerdict "findings_present"/,
    );
  });
});

test("#2389 negative: an arbitrary blocked over a completed ledger is refused when no blocker is proven", async () => {
  for (const overallVerdict of ["clean", "findings_present"]) {
    await withCompositionRound({ overallVerdict, prBody: FULLY_TICKED_PR_BODY }, async ({ post }) => {
      await assert.rejects(() => post({ verdict: "blocked" }), (err) => {
        assert.match(err.message, new RegExp(`contradicts the consolidated ledger's overallVerdict "${overallVerdict}"`));
        assert.match(err.message, /No deterministic pre-approval blocker is proven/);
        return true;
      });
    });
  }
...
test("#2389: the review-verdict and gate-blockers lines leave the parsed gate/head/verdict identity unchanged", () => {
  const base = {
    gate: "pre_approval_gate", headSha: GATE_FULL_HEAD, verdict: "blocked",
    findingsSummary: "review round summary", nextAction: "rerun gate",
    executionMode: "inline_single_agent", inlineReason: "composition test",
  };
  const plain = renderGateReviewCommentBody(base);
  const composed = renderGateReviewCommentBody({
    ...base,
    reviewVerdict: "clean",
...
test("#1621: an inline free-text-only findings_present round (no structuredFindings, no counts) escalates gate:full via the verdict fallback", async () => {
  ...
  try {
    ...
    const result = await upsertCheckpointVerdict({
      ...
      nextAction: "stay draft and fix",
      executionMode: "inline_single_agent", inlineReason: "inline free-text test",
    }, { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", repoRoot, runChild });
    assert.equal(result.action, "created");
    assert.equal(result.gateFullLabelApplied, true);
  ...
  try {
    const draftGateComment = {
      ...
      body: renderGateReviewCommentBody({
        gate: "draft_gate", headSha: GATE_FULL_HEAD, verdict: "findings_present",
        findingsSummary: "a high-severity finding remains", nextAction: "stay draft and fix",
        executionMode: "inline_single_agent", inlineReason: "inline noop-retry test",
      }),
      html_url: "https://github.com/owner/repo/pull/17#issuecomment-91",
      updated_at: "2026-06-01T00:00:00Z",
    };
...
// ---------------------------------------------------------------------------
// review-gate same-head correction: a submitted review-gate review is the
// round's existing surface, so a same-head rerun updates it in place or is a
// noop instead of POSTing a duplicate submitted review. A correction needing a
// NEW inline comment fails closed (GitHub cannot add one to a submitted
// review); --new-round forces a fresh review round.
// ---------------------------------------------------------------------------

const REVIEW_GATE_DIFFERENT_HEAD = "def4567000000000000000000000000000000000";

// Run a --gate review create and return the exact body it POSTed, so a later
// same-head rerun can seed a byte-identical submitted review.
async function createReviewGateAndCaptureBody(tempDir, { nextAction }) {
  const ledgerPath = await writeReviewGateLedger(tempDir, [], { verdict: "clean", overallVerdict: "clean" });
  const { runChild, calls } = makeGhMock([
    ...reviewGateFindingSurfaceEntries({ files: null }),
    {
  ...
  assert.equal(result.action, "created");
  const postCall = calls.find((c) => c.args.includes("repos/owner/repo/pulls/17/reviews") && c.args.includes("POST"));
  return { ledgerPath, body: JSON.parse(postCall.stdinText).body };
}

function seededSubmittedReview({ id, body, commitId = SINGLE_SURFACE_HEAD, login = "gate-bot", state = "COMMENTED" }) {
  return { id, state, commit_id: commitId, user: { login }, html_url: `https://github.com/owner/repo/pull/17#pullrequestreview-${id}`, body };
}

test("#2030: --gate review same-head rerun with a matching submitted review is a NOOP (no second review)", async () => {
  await withTempDir(async (tempDir) => {
    const nextAction = "none — informational review, no re-gate required";
    const { ledgerPath, body } = await createReviewGateAndCaptureBody(tempDir, { nextAction });
    ...
    assert.equal(result.surface, "review");
    // No create POST and no PUT — the submitted review is left as-is.
    assert.equal(calls.filter((c) => c.args.includes("POST") || c.args.includes("PUT")).length, 0);
  }, { prefix: "dev-loops-upsert-review-samehead-noop-" });
});

test("#2030: --gate review same-head rerun with a body-only correction UPDATES the submitted review in place (no duplicate)", async () => {
  await withTempDir(async (tempDir) => {
    ...
    const result = await upsertCheckpointVerdict({
      ...
      gate: "review",
      headSha: SINGLE_SURFACE_HEAD,
      // Changed next action → the rendered body differs, but no new inline
      // comment is required, so the submitted review body is updated in place.
    ...
    assert.equal(result.action, "updated");
    assert.equal(result.commentId, 980);
    assert.equal(calls.filter((c) => c.args.includes("PUT") && c.args.includes("repos/owner/repo/pulls/17/reviews/980")).length, 1);
    // No create POST — the existing review was corrected, not duplicated.
    assert.equal(calls.filter((c) => c.args.includes("POST") && c.args.includes("repos/owner/repo/pulls/17/reviews")).length, 0);
...
test("#2030: --gate review correction that needs a NEW inline comment on a submitted review FAILS CLOSED (names the constraint and --new-round), no duplicate", async () => {
  await withTempDir(async (tempDir) => {
    ...
    const submitted = seededSubmittedReview({ id: 982, body });
    // A ledger carrying a locatable finding not yet posted → a new inline
    // comment is required, which GitHub cannot add to a submitted review.
    const ledgerPath = await writeReviewGateLedger(tempDir, [LOCATABLE_FINDING], { overallVerdict: "findings_present" });
    const { runChild, calls } = makeGhMock(
      reviewGateFindingSurfaceEntries({
        files: [{ filename: "src/db.mjs", patch: SINGLE_SURFACE_PATCH }],
    ...
    await assert.rejects(
      upsertCheckpointVerdict({
        ...
        headSha: SINGLE_SURFACE_HEAD,
        verdict: "findings_present",
        findingsSummary: "one locatable finding",
        nextAction: "stay draft and fix",
        findingsLedger: ledgerPath,
        executionMode: "fanout_fanin",
        submit: "comment",
      }, { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", runChild, repoRoot: tempDir }),
      /already-submitted review 982[\s\S]*inline[\s\S]*--new-round/i,
    );
    // No create POST — the round fails closed instead of duplicating.
    assert.equal(calls.filter((c) => c.args.includes("POST") && c.args.includes("repos/owner/repo/pulls/17/reviews")).length, 0);
  }, { prefix: "dev-loops-upsert-review-samehead-failclosed-" });
});

test("#2030: --gate review --new-round forces a FRESH review even when a same-head submitted review exists (distinct id)", async () => {
  await withTempDir(async (tempDir) => {
    const { body } = await createReviewGateAndCaptureBody(tempDir, { nextAction: "none — informational review, no re-gate required" });
    const submitted = seededSubmittedReview({ id: 983, body });
    const ledgerPath = await writeReviewGateLedger(tempDir, [], { verdict: "clean", overallVerdict: "clean" });
    // --new-round bypasses findOwnSubmittedReview entirely, so the mock carries
    // only the normal create sequence (pending scan → finding surface → POST),
    // never the submitted-review scan.
    const { runChild, calls } = makeGhMock([
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/reviews?per_page=100"], stdout: JSON.stringify([submitted]) + "\n" },
      { assertArgs: ["api", "user"], stdout: '{"login":"gate-bot"}\n' },
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/reviews?per_page=100"], stdout: JSON.stringify([submitted]) + "\n" },
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/issues/17/comments?per_page=100"], stdout: "[]\n" },
      {
      ...
      {
        assertArgs: ["api", "-X", "POST", "repos/owner/repo/pulls/17/reviews", "--input", "-"],
        stdout: '{"id":984,"html_url":"https://github.com/owner/repo/pull/17#pullrequestreview-984"}\n',
      },
    ]);
    const result = await upsertCheckpointVerdict({
      repo: "owner/repo",
    ...
    assert.equal(result.action, "created");
    assert.equal(result.commentId, 984);
    assert.notEqual(result.commentId, 983);
    assert.equal(calls.filter((c) => c.args.includes("POST") && c.args.includes("repos/owner/repo/pulls/17/reviews")).length, 1);
  }, { prefix: "dev-loops-upsert-review-new-round-" });
});

test("#2030: findOwnSubmittedReview matches only the caller's own same-head review-gate review", async () => {
  const headSha = SINGLE_SURFACE_HEAD;
  const validBody = "### Gate review: `review`\n\n**Reviewed head SHA:** `abc1234000000000000000000000000000000000`\n";
  const gh = (reviews) => {
    const { runChild } = makeGhMock([
      { assertArgs: ["api", "--paginate", "--slurp", "repos/owner/repo/pulls/17/reviews?per_page=100"], stdout: JSON.stringify(reviews) + "\n" },
      { assertArgs: ["api", "user"], stdout: '{"login":"gate-bot"}\n' },
    ], { repeatLastOnOverflow: true });
    return { env: runIdFreeEnv({ DEVLOOPS_RUN_ID: "" }), ghCommand: "gh", runChild };
  };

  // Own, same-head, review-gate header → matched.
  const own = { id: 500, state: "COMMENTED", commit_id: headSha, user: { login: "gate-bot" }, html_url: "https://x/500", body: validBody };
  assert.deepEqual(
    await findOwnSubmittedReview({ repo: "owner/repo", pr: 17, headSha }, gh([own])),
    { id: 500, body: validBody, commentUrl: "https://x/500" },
  );
  // Foreign author → null (GitHub lists every submitted review).
  assert.equal(await findOwnSubmittedReview({ repo: "owner/repo", pr: 17, headSha }, gh([{ ...own, user: { login: "someone-else" } }])), null);
  // Wrong head → null (a review on a different commit_id is not this round's surface).
  assert.equal(await findOwnSubmittedReview({ repo: "owner/repo", pr: 17, headSha }, gh([{ ...own, commit_id: REVIEW_GATE_DIFFERENT_HEAD }])), null);
  // No review-gate header → null.
  assert.equal(await findOwnSubmittedReview({ repo: "owner/repo", pr: 17, headSha }, gh([{ ...own, body: "just a normal review comment" }])), null);
  // Still PENDING → null (findOwnPendingReview owns that surface).
  assert.equal(await findOwnSubmittedReview({ repo: "owner/repo", pr: 17, headSha }, gh([{ ...own, state: "PENDING" }])), null);
});

test("#2030: --new-round is scoped to --gate review only", () => {
  const base = ["--repo", "owner/repo", "--pr", "17", "--head-sha", "abc1234000000000000000000000000000000000", "--verdict", "clean", "--findings-summary", "x", "--next-action", "y", "--execution-mode", "fanout_fanin"];
  assert.throws(
    () => parseUpsertCheckpointVerdictCliArgs([...base, "--gate", "draft_gate", "--new-round"]),
    /--new-round is scoped to --gate review only/i,
  );
