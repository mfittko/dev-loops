...
test("a finding that is not an object or lacks a non-empty string summary, canonical severity, or angle is malformed in any execution mode", async () => {
  const inlineMarker = { ...PA_MARKER, executionMode: "inline_single_agent" };
  for (const bad of [null, "act", ["x"], { severity: "low", angle: "x" }, { severity: "low", angle: "x", summary: "" }, { severity: "low", angle: "x", summary: "  " },{ severity: "urgent", angle: "x", summary: "s" }, { severity: "low", summary: "s" }, { severity: "low", angle: " ", summary: "s" }]) {
    for (const marker of [PA_MARKER, inlineMarker]) {
