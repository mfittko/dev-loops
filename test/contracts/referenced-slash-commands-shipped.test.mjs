...
// hyphens). The negative lookbehind excludes path-adjacent occurrences so a
// file path like `commands/loop-continue.command.md` or
// `.claude/commands/loop-continue.md` (where `/loop-...` is preceded by a
// word/path char) is NOT mistaken for an instruction. The namespaced form
// `/dev-loops:loop-continue` contains no `/loop-` substring at all (the
// `loop-continue` there follows `:`), so it never matches.
const BARE_LOOP_CMD_RE = /(?<![-A-Za-z0-9_\/.\\:])\/loop-([a-z][a-z-]*)/g;

// For each bare `/loop-<cmd>` token in a file, the file must also contain the
// namespaced plugin spelling `/dev-loops:loop-<cmd>` (same command root).
// Returns the human-readable failure strings (one per uncovered command root).
...
// A bare `/loop-continue` with no namespaced form is flagged; adding
// `/dev-loops:loop-continue` clears it. A file path reference
// (`commands/loop-continue.command.md`) is NOT flagged (path-adjacent).
test("computeBareSlashFailures flags an uncovered bare reference, clears it with the namespaced form, and ignores file paths", () => {
  const surfaceFile = path.join(REPO_ROOT, "skills/dev-loop/SKILL.md");

  const bareOnly = computeBareSlashFailuresFromContent(surfaceFile, "run `/loop-continue #N` to proceed");
  assert.equal(bareOnly.length, 1);
  assert.match(bareOnly[0], /instructs a bare `\/loop-continue`/);

  const bothSpellings = computeBareSlashFailuresFromContent(surfaceFile, "run `/dev-loops:loop-continue #N` (or `/loop-continue #N` in the dev-loops repo) to proceed");
  assert.equal(bothSpellings.length, 0);

  const filePath = computeBareSlashFailuresFromContent(surfaceFile, "see commands/loop-continue.command.md and .claude/commands/loop-continue.md");
  assert.equal(filePath.length, 0);

  const cliReplacement = computeBareSlashFailuresFromContent(surfaceFile, "run `dev-loops loop continue #N` to proceed");
  assert.equal(cliReplacement.length, 0);
});

// Helper variant that drives the same logic from in-memory content (so the
// negative self-test does not depend on the repo's current state).
function computeBareSlashFailuresFromContent(surfaceFile, content) {
