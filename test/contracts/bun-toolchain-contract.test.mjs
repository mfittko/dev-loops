...
test("durable guidance records the toolchain and accepted benchmark", async () => {
  ...
  assert.match(await read("docs/decisions/0060-bun-development-toolchain.md"), /Bun 1\.4\.1[\s\S]*Node `>=24`[\s\S]*npm/i);
  const benchmark = await read("docs/benchmarks/bun-1.4.1/verdict.md");
  assert.match(benchmark, /Verdict: \*\*pass\*\*/);
