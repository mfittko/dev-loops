...
// in-flight checkpoint, only in the linked worktree's own tmp/. With "both",
// a complete main checkpoint and a stale incomplete linked one exist for the
// same head: the main-anchored copy wins, so the gate is not blocked.
for (const location of ["main", "linked", "both"]) test(`detect-pr-gate-coordination-state ${location === "both" ? "reads the complete main checkpoint over a stale incomplete linked one" : "blocks on an incomplete fixer-disposition checkpoint"} for the current head (checkpoint in ${location} worktree)`, async () => {
  ...
  try {
    ...
    const fullPath = path.join(location === "main" ? tempDir : linked, logPath);
    ...
    if (location === "both") {
      // Complete: T1 is deferred, so nothing tackled needs live evidence.
      const mainPath = path.join(tempDir, logPath);
    ...
    const env = await writeGhStub(tempDir, [
      ...
      // No tackled entry in the "both" main checkpoint, so no containment call.
      ...(location === "both" ? [] : [{
...
test("detect-pr-gate-coordination-state formats a legible reason (no raw null threadId) when the fixer-disposition checkpoint is malformed", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "dev-loops-pr-gate-fixer-disposition-malformed-"));
  const REPO = "owner/repo";
  const PR = 3003;
  const HEAD_SHA = "abc1122334";
  ...
  try {
    ...
    await mkdir(path.dirname(fullPath), { recursive: true });
    // Malformed: not valid JSON at all, so resolveFixerDispositionInput's
    // normalizeFixerDispositionHandoff(JSON.parse(raw)) throws before any
    // threadId is ever known.
    await writeFile(fullPath, "{ not valid json", "utf8");
    ...
    assert.equal(result.ok, true);
    assert.equal(result.nextAction, PR_CHECKPOINT_ACTION.COMPLETE_FIXER_DISPOSITION);
    assert.match(result.reason, /unreadable_checkpoint/);
    assert.doesNotMatch(result.reason, /thread null(?!\w)/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});

test("detect-pr-gate-coordination-state fails closed (not silently absent) when the fixer-disposition checkpoint exists but is unreadable", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "dev-loops-pr-gate-fixer-disposition-unreadable-"));
  const REPO = "owner/repo";
  const PR = 3004;
  const HEAD_SHA = "abc9998887";

  try {
    const { buildLogPath } = await import("../../scripts/github/write-gate-findings-log.mjs");
    const logPath = buildLogPath({ repo: REPO, pr: PR, gate: "fixer-disposition", headSha: HEAD_SHA, tmpRoot: "tmp" });
    const fullPath = path.join(tempDir, logPath);
    // Simulate a genuinely unreadable checkpoint (EACCES/EIO-shaped, not
    // ENOENT) by making the checkpoint path a directory instead of a file:
    // readFile() throws EISDIR, a non-ENOENT error the resolver must fail
    // closed on, distinct from a genuine absence (ENOENT).
