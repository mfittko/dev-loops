...
test("a fully-rewritten post-grill description passes the write-back hygiene checks and still trips detectIssueRefinementArtifact", () => {
  const rewrittenBody = [
    ...
    "|---|---|",
    "| long reasons are truncated with an ellipsis | a focused test proves over-limit reasons are truncated |",
    "| short reasons are left untouched | a test proves at-or-under-limit reasons are unchanged |",
    "",
    "## Non-goals",
    "",
