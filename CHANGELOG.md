...
- **Decks no longer clip slide content on mobile (~390px)** (#937). On a phone, a wide grid/flex child — the inline flow diagram (`.flow { min-width: max-content }`, `.node { white-space: nowrap }`) — forced the grid track wider than the viewport; because the cell lacked `min-width: 0`, the track (and its sibling text card, stretched to the same row width) blew past 390px and `body { overflow-x: hidden }` + `.slide { overflow: hidden }` **clipped** it (bullets cut mid-word, flow nodes sheared). Fix (both `docs/presentations/applied-dev-loops.html` and `process-observability.html`): add `min-width: 0` to grid children (`.grid > *`, `.grid > .glass-card/.metric-card`) and to `.flow-scroll` (the standard grid/flex blowout fix), so the wide diagram scrolls *within* its existing `.flow-scroll { overflow-x: auto }` card instead of forcing the slide wide; add `overflow-wrap: anywhere` to `p`/`li` and `code` so long tokens wrap. The CSS lives only in the HTML renders (the Slidev `*-presentation.md` sources carry none of it), so no `.md` change was needed. The dark visual identity, section ids, CSP guard, and reduced-motion guard are unchanged. The UI-gate blind spot is closed: both Playwright deck specs (`test/playwright/applied-deck.spec.mjs`, `observability-deck.spec.mjs`) now run a **mobile (390×844)** pass that walks in-flow elements and fails if any element's `getBoundingClientRect().right` exceeds `innerWidth + 1` unless it sits inside an `overflow-x:auto` scroller — an assertion `overflow:hidden` can't mask (the prior `body.scrollWidth` check could). The new assertion demonstrably fails on the pre-fix layout and passes after; each spec also captures one mobile named state.
