...
- Durable rules and lessons live in skill/contract prose or deterministic tooling — never in agent memory alone; never amend a contract to bless our own drift, and check artifacts against their external spec, not each other (`STYLE-DURABLE-RULE-HOME`, `STYLE-NO-DRIFT-BLESSING`, `STYLE-CONSISTENCY-NOT-CONFORMANCE` in `skills/docs/contract-style-guide.md`).
