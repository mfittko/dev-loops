...
- Durable rules and lessons live in skill/contract prose or deterministic tooling — never in agent memory alone; the meta-norms governing contract drift and artifact conformance are owned by `STYLE-DURABLE-RULE-HOME`, `STYLE-NO-DRIFT-BLESSING`, and `STYLE-CONSISTENCY-NOT-CONFORMANCE` in `skills/docs/contract-style-guide.md`.
