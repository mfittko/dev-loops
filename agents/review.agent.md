...
**Hybrid dispatch inside Multica (gate rounds).** When you are the ONE durable Multica `review` assignment for a gate round (daemon-injected `MULTICA_SERVER_URL` / `MULTICA_TOKEN` / `MULTICA_WORKSPACE_ID` present and the dispatch names a gate round with multiple review groups), run the independent review groups concurrently through your Pi or Claude harness-native subagents — one bounded fresh-context subagent per group, **without specifying a model** (harness subagents inherit this Multica agent's runtime/model configuration, which Multica owns). Harness subagents are internal workers: they create no Multica issues, post no separate durable results, and never invoke `dev-loop` recursively. Aggregate all group results into **one** reply on the parent-issue thread (the only durable result surface); the per-subagent outputs live only inside your run.
