...
4. **Package-relative (legacy):** `../..` from this agent's own directory (the original package-local install layout).
5. **Global npm root:** `$(npm root -g)/dev-loops`.

NEVER fall back to `find /` or any unbounded filesystem walk to locate the CLI — it stalls and trips the needs-attention timeout. If every bounded candidate fails, stop and ask the orchestrator/operator for the dev-loops package root rather than searching. (The `dev-loop` skill resolves it analogously — same checkout-first walk-up, same "checkout wins, no version comparison" rule.)
<!-- /pi-only -->
...
- Every sub-loop (refine, implement, gate review round, fixer) runs in its own dedicated fresh-context agent. A gate review round runs in one gate coordinator agent per round, which owns the primer, reviewer fan-out, fan-in, and judge (`GATE-EXEC-GATE-COORDINATOR` in [Gate Review Sub-Loop Contract](../skills/docs/gate-review-sub-loop-contract.md)). An agent dispatched to run one of these sub-loops executes it as its assigned bounded task. The coordinator keeps its own context thin and performs no sub-loop work inline.
- No child subagent spawning beyond assigned fan-out work; a gate coordinator also dispatches the round's judge (`GATE-EXEC-GATE-COORDINATOR`).
...
- **`todo` tool gap under Pi (#1583, #1604):** Pi has no `todo` builtin (Claude keeps `TodoWrite`). The dev-loop entrypoint source (`agents/dev-loop.agent.md`) no longer declares `todo` or `agent` (#1604): `agent` is redundant with `subagent` (already listed), and `todo` has no Pi builtin. Role-agent sources (`fixer`, `developer`, `docs`, `quality`, `refiner`) keep the neutral `search`/`execute` vocabulary unchanged — both harnesses map them (Claude `search`→Grep+Glob; Pi sync `search`→`bash`), and dropping them would regress Claude (#1086). The `review` agent is exempted per #1659: it drops `search`/`execute` from source so Pi does not mark review fan-out steps `failed` (which aborts `runs.all` / GATE-EXEC-PRIME); the review agent uses `bash` (rg/grep) for search on both harnesses. Track the acceptance checklist via prose/bash under Pi — emit the structured acceptance report directly rather than relying on a todo tool.
