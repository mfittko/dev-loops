...
**Hybrid dispatch for gate rounds (Multica):** a gate round is exactly one durable Multica `review` assignment on the parent issue. That `review` agent fans its review groups out through its Pi or Claude harness-native subagents (concurrent, fresh-context), aggregates them, and posts one parent-issue result. Harness subagents are internal workers — no Multica issues, no separate durable results, no recursive `dev-loop`, and no model override (they inherit the Multica-governed runtime/model).
