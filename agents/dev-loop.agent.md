...
- Every sub-loop (refine, implement, gate review round, fixer) runs in its own dedicated fresh-context agent. A gate review round runs in one gate coordinator agent per round, which owns reviewer fan-out, fan-in, and judge (`GATE-EXEC-GATE-COORDINATOR` in [Gate Review Sub-Loop Contract](../skills/docs/gate-review-sub-loop-contract.md)). An agent dispatched to run one of these sub-loops executes it as its assigned bounded task. The coordinator keeps its own context thin and performs no sub-loop work inline.
